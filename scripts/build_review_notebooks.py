...
STAGES: list[dict] = [
        ...
        plots=[
            dict(
                ...
                code=(
                    ...
                    "    cols = ['tab:green' if m == 'psffit' else '0.6' for m in methods]\n"
                    ...
                    "    ax.set_title(f\"E3 · Ṁ por método (canónico psffit=8.2e-13; scatter {q['intermethod_scatter_pct']:.0f}%)\")\n"
                    "    ax.axhline(lim['psffit']['mdot'], color='tab:green', ls='--', lw=1); fig.tight_layout()\n"
            ...
            dict(
                ...
                code=(
                    ...
                    "    import matplotlib.pyplot as plt\n"
                    "    q = nb.load_qc('stages/stage_h03_qc.json', RUN_ID)\n"
                    "    p = {L['method']: L for L in q['limits']}['psffit']; pin = q['physical_inputs']\n"
                    ...
                    "    ax.set_title('E3 · cómo la no-detección se vuelve Ṁ (método canónico psffit)')\n"
                    "    fig.tight_layout()\n"
                    "    outdir = nb.run_dir(RUN_ID) / 'plots' / 'e3_limits'; outdir.mkdir(parents=True, exist_ok=True)\n"
    ...
    dict(
        ...
        evidence_code=(
            ...
            "import pandas as pd\n"
            "d = pd.read_csv(nb.run_dir(RUN_ID) / 'report' / 'characterization' / 'final_line_table.csv')\n"
            "print(f\"\\nz_score de las 24 líneas: {d['z_score'].min():.2f} .. {d['z_score'].max():.2f} (todas < 5σ)\")"
        ),
        plots=[
            dict(
                md=(
                    "## Plot 1 — las 24 líneas: ninguna detectada\n\n"
                    "El `z_score` (significancia de detección) de cada línea, del "
                    "`final_line_table.csv`, coloreado por familia. **Todas dentro de ±5σ** (de hecho "
                    "|z|<4) → 0 detectadas. Hα marcada con ★."
                ),
                code=(
                    "try:\n"
                    "    import numpy as np\n"
                    "    import pandas as pd\n"
                    "    import matplotlib.pyplot as plt\n"
                    "    d = pd.read_csv(nb.run_dir(RUN_ID) / 'report' / 'characterization' / 'final_line_table.csv').sort_values('rest_A')\n"
                    ...
                    "        z = r['z_score']\n"
                    "        ax.plot(z if pd.notna(z) else 0, i, 'o', color=cm.get(r['family'], '0.5'), ms=7)\n"
                    ...
                    "    ax.set_title('G2 · 24 líneas: 0 detectadas, 23 límites, 1 no medible (todas |z|<5)')\n"
                    "    ax.legend(fontsize=8, loc='lower right'); fig.tight_layout()\n"
            ...
            dict(
                md=(
                    ...
                    "El límite superior de flujo (5σ) de cada línea medible (escala log), del "
                    "`final_line_table.csv`. Es el **producto** que G3 consume; Hα (★) es la más "
    ...
    dict(
        ...
        evidence_code=(
            ...
            "    tag = '  <- EMPATE (líder)' if h['log_l_rel'] == 0.0 else ('  (excluido)' if h['log_l_rel'] <= -4 else '')\n"
            "    print(f\"   {h['hypothesis']:22s} {h['log_l_rel']:8.0f}{tag}\")"
        ),
        plots=[
            dict(
                md=(
                    ...
                    "indistinguibles; **artefacto, contaminante y fondo excluidos** (rojo). Es "
                    "compañero real, pero el subtipo queda ambiguo."
                ),
                code=(
                    "try:\n"
                    "    import matplotlib.pyplot as plt\n"
                    ...
                    "    cols = ['tab:green' if v == 0.0 else ('tab:red' if v <= -4 else '0.6') for v in ll]\n"
                    "    fig, ax = plt.subplots(figsize=(9, 4.5))\n"
                    "    y = range(len(names))\n"
                    ...
                    "        else:\n"
                    "            tag = f\"{v:.0f}\" + ('  (excluido)' if v <= -4 else '')\n"
    ...
    dict(
        ...
        evidence_code=(
            ...
            "print('\\nopen_issue (blocking):', q['open_issues'][0]['issue'][:120])"
        ),
        plots=[
            dict(
                md=(
                ...
                code=(
                    ...
                    "        f\"        compañero real ligado a ROXs 12 A (sep 1.80\\\", PA 240°)\",\n"
                    "        '',\n"
                    "        f\"ACRECIÓN (no-detección de Hα):\",\n"
                    "        f\"   L_acc  ≤ 4.1e-6 L☉        (Alcalá+2017 Hα)\",\n"
                    ...
                    "        f\"   masa       17.5 M_Jup    (Bowler+2017, hot-start)\",\n"
                    "    ]\n"
                    ...
                    "    ax.text(0.5, 0.03, 'PROVISIONAL: A-block abierto + tipado espectral (G3) diferido -> subtipo ambiguo',\n"
            ...
            dict(
                ...
                code=(
                    ...
                    "        ('A-block cerrado (paper-válido)', False),\n"
                    "    ]\n"
                    ...
                    "    ax.set_xlim(0, 1); ax.set_xticks([]); ax.set_yticks([]); ax.invert_yaxis()\n"
                    "    ax.set_title('G5 · integridad del paquete: determinista y trazable, provisional por el A-block')\n"
                    "    fig.tight_layout()\n"
        ...
        conclusion_md=(
            ...
            "intacto (V6).\n"
            "- **Bloqueo heredado:** A-block abierto (alineación, M3, M5) + tipado espectral de G3 "
            "diferido → **nada es paper-final hasta cerrar el A-block**.\n"
            "- **Para cerrar:** A-block + G3 real (librerías BT-Settl/BHAC15/Luhman-Bonnefoy) + 2ª "
