        ...
        plots=[
            ...
            dict(
                md=(
                    ...
                    "Renderizados **directamente del FITS** `stageS0_offset_map.fits` (ext "
                    "`OFFSET_A`, `ERR_A`, `OFFSET_CH`, `ERR_CH`), para **ambos cubos** "
                    "(realineado y ADP-control) — no depende del PNG pre-generado. Cada fila "
                    "es un cubo con 4 paneles: mapa de offset (Å), mapa de error (Å), perfil "
                    "colapsado por columna (dirección de los stripes) y el histograma de "
                    "offsets sobre los spaxels de bajo error."
                ),
                code=(
            ...
            dict(
                md=(
                    ...
                    "(el p95 crudo y el `median|offset|` los dominan). Si hubiera un offset "
                    "**coherente pequeño escondido en el ruido**, la forma de sacarlo a la luz "
                    "es medirlo donde la S/N es máxima: el halo de la estrella central. Este "
                    "paso recorta un **100×100 centrado en la estrella** (centroide de los "
                    "spaxels de menor error, sin cargar el cubo) y recalcula las métricas de "
                    "S0 ahí, más el **offset medio con signo ± error** — el test directo de un "
                    "offset coherente oculto (que el `median|·|` no ve porque no distingue "
                    "signo).\n\n"
                    "Se compara *full-frame* vs *crop* para ambos cubos; la conclusión robusta "
                    "es que en el crop (i) el `median|offset|` cae al quitar el ruido de "
                    "bordes, y (ii) `stripe_sig` sigue ≤ el control transversal → **ningún "
                    "stripe alineado con slicers aparece al subir la S/N**; lo que queda es un "
                    "zero-point casi uniforme (unas decenas de mÅ, ≲0.04 canal), coherente con "
                    "la interpretación temporal del G1 y con M1 (+74 mÅ)."
                ),
                code=(
                    "import numpy as np, matplotlib.pyplot as plt\n"
                    "from astropy.io import fits\n"
