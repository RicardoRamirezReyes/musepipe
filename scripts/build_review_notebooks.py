        ...
        plots=[
            ...
            dict(
                md=(
                    ...
                    "**Por qué mirar por exposición:** el cubo combinado promedia acimutalmente "
                    "(el campo rota 19.3° entre exposiciones) ⇒ un stripe fijo del slicer se "
                    "diluye. Cada exposición conserva la orientación del slicer, así que 0/7 con "
                    "estructura de slicer es la prueba real de que no hay stripes."
                ),
                code=(
            ...
            dict(
                md=(
                    ...
                    "Dos lecturas: (1) el **offset medio del crop por exposición** traza la "
                    "**deriva temporal** de zero-point (modo TEMPORAL de G1/S3); (2) `stripe_sig` "
                    "≤ el control transversal en todos ⇒ **ningún stripe de slicer** aun al "
                    "máximo S/N (modo ESPACIAL descartado).\n\n"
                ...
                code=(
                    ...
                    "      'caso/región','n_lowE','med|o|','p95 Å','meanmÅ','σ_mean','strp','trnv'))\n"
                    "rows = []; crops = []\n"
                    ...
                    "              mm['p95_abs_offset_A'], mn*1e3, se*1e3, mm['structure_significance'], mm['transverse_significance']))\n"
                    "    rows.append((label, meanC*1e3, seC*1e3, mC['structure_significance'], mC['transverse_significance']))\n"
                    "    crops.append((label, oc[sl].copy(), oa[sl].copy(), ec[sl].copy(), step, thr, meanC, seC))\n"
                    ...
                    "ax[1].set_xlabel('transverse_sig (control)'); ax[1].set_ylabel('stripe_sig'); ax[1].set_xlim(0,limmax); ax[1].set_ylim(0,limmax)\n"
                    "ax[1].set_title('stripe vs control transversal (bajo la diagonal = sin stripe)'); ax[1].legend(fontsize=7)\n"
                    "fig.tight_layout(); plt.show()\n"
                    "# --- detalle 4-panel del CROP por caso (como la celda de mapas, pero recortado) ---\n"
                    "print('\\nDetalle 4-panel del crop (offset / error / perfil por columna / histograma) por caso:')\n"
                    "for label, oc_c, oa_c, ec_c, step, thr, meanC, seC in crops:\n"
                    "    lo = np.isfinite(oa_c) & np.isfinite(ec_c) & (ec_c < thr)\n"
                    "    prof = wsm.stripe_profile(oc_c, 'vertical')\n"
                    "    fig, ax = plt.subplots(1, 4, figsize=(16, 3.2))\n"
                    "    fig.suptitle(f'S0 · {label} · CROP 100×100 (n bajo error={int(lo.sum())}, media {meanC*1e3:+.1f}±{seC*1e3:.1f} mÅ)', fontsize=10)\n"
                    ...
                    "    ax[2].plot(np.arange(p_A.size), p_A, lw=0.9); ax[2].axhline(0, color='0.6', lw=0.6); ax[2].set_title('perfil por columna (∥ stripes)'); ax[2].set_xlabel('columna (crop)'); ax[2].set_ylabel('offset mediano (Å)')\n"
                    "    if lo.any():\n"
