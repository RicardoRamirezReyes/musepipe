        ...
        plots=[
            dict(
                md=(
                    ...
                    "**Una fila por método, `continuo − offset`.** En gris lo que hay en la apertura "
                    "sin restar nada; en color lo que deja el método. A la referencia se le resta su "
                    "**valor mínimo** (el offset, que va escrito en la leyenda): sin eso el pedestal "
                    "la manda arriba del todo y lo único que se aprende es que está muy por encima; "
                    "con el mínimo fuera, lo que queda en el eje es su **forma**, que es lo que se "
                    "puede comparar con la del método. El eje es symlog porque el residuo cruza el "
                    "cero.\n\n"
                    "**Las tres curvas verdes** son la misma medida repetida donde **no hay "
                    "compañero**: al otro lado de la primaria y en los dos perpendiculares, a la "
                    "misma separación (colocadas con los helpers de B3, así que su PA está en la "
                    "convención del run). Son **halo puro**, y llevan el mismo offset para seguir "
                    "siendo comparables. Sirven para dos cosas:\n\n"
                    "- lo que separa al gris de ellas **es el compañero** — se ve a partir de "
                    "~8500 Å, donde el gris sube y las verdes bajan;\n"
                    "- su dispersión mide **cuánto cambia el halo con el ángulo** a esa misma "
                    "separación. En ROXs 12 b los continuos medianos van de 3778 (posición opuesta) a "
                    "4317: una variación azimutal comparable a lo que aporta el compañero en esa "
                    "apertura. Eso es, en una imagen, por qué la sustracción de halo domina esta "
                    "etapa.\n\n"
                    "*(La celda construye el modelo de PSF: tarda ~27 s. La referencia se calcula "
                    "**una vez** y se reutiliza en el plot siguiente.)*"
                ),
                code=(
                    "try:\n"
                    "    import matplotlib.pyplot as plt\n"
                    "    from musepipe.stages import unsubtracted_aperture_reference, halo_removal_figure\n"
                    "    rd = nb.run_dir(RUN_ID)\n"
                    "    # Se guarda en `ref_sin_sustraer` para no recalcularla en el plot siguiente.\n"
                    ...
                    "        fig, axes = halo_removal_figure(rd / 'stages', plt=plt, reference=ref_sin_sustraer)\n"
            ...
            dict(
                md=(
                    "## Plot 2 — lo que QUEDA tras restar, en % de lo que había\n\n"
            ...
            dict(
                md=(
                    "## Plot 3 — ¿el continuo del compañero es suyo o es halo de la primaria?\n\n"
                    "### La pregunta física\n\n"
            ...
            dict(
                md=(
                    "## Plot 4 — el presupuesto de error\n\n"
            ...
            dict(
                md=(
                    "## Plot 5 — los espectros definitivos (6 métodos + la primaria)\n\n"
