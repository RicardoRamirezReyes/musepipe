...
STAGES: list[dict] = [
    ...
    dict(
        ...
        narrative_md=(
            ...
            "`a1a_molecfit_crosscheck` — en el `stage00r_qc.json` del run **realineado**, que "
            "es el que lo trae; el del crudo no— y en `a3_telluric_justification.md §6`. "
            "Desde A3 v3 hay además una comparación que emite la propia etapa, en "
            "`stage00t_qc.json → arbiter`.\n\n"
        ...
        evidence_code=(
            ...
            "    mf = cc['model_fit']\n"
            "    print(f\"  converge: mpfit status={mf['mpfit_status']}, \"\n"
            "          f\"rel_col_O2={mf['rel_mol_col_O2']}+-{mf['rel_mol_col_O2_unc']}, \"\n"
            "          f\"ppmv_O2={mf['ppmv_O2']:.0f}\")\n"
            "except (FileNotFoundError, KeyError) as e:\n"
            "    print('  (no disponible para esta cadena:', type(e).__name__, e, ')')\n"
            "print()\n"
            "print('Las dos vias, si A3 v3 ya arbitro en esta cadena:')\n"
            "try:\n"
            "    qt = nb.load_qc('stages/stage00t_qc.json', RUN_ID)\n"
            "    arb = qt['arbiter']\n"
            "    for via, fom in sorted(arb['fom_by_method'].items(), key=lambda kv: kv[1]):\n"
            "        print(f'  {via:22s} FoM {fom:.4f}')\n"
            "    veredicto = arb['method']\n"
            "    sigma = arb.get('tie_sigma')\n"
            "    print(f\"  => gana {veredicto['winner']} ({veredicto['reason']}), margen \"\n"
            "          f\"{veredicto['margin']:.4f}, sigma de empate {sigma}\")\n"
            "    print('  ventanas ajustadas:', qt['fit'].get('regions_bands'),\n"
            "          '| excluidas por someras:', qt['fit'].get('regions_excluded_shallow'))\n"
            "except (FileNotFoundError, KeyError) as e:\n"
            "    print('  (esta cadena aun no tiene A3 v3:', type(e).__name__, e, ')')"
        ),
        plot_md=(
        ...
        plots=[
            ...
            dict(
                md=(
                    ...
                    "- **centro:** el **compañero por los 6 métodos**. El dato **por canal** de los "
                    "seis va detrás en gris —es la amplitud real del ruido— y encima la mediana "
                    "móvil de `SUAVIZADO_CH` canales, que es lo único que deja leer seis espectros "
                    "a la vez; debajo, la banda de error total del canónico, sin suavizar. Pon "
                    "`SUAVIZADO_CH = 1` y quedan las seis curvas crudas, sin gris ni mediana. "
                    "Hα en rojo punteado.\n"
                ...
                code=(
                    "# Ancho de la mediana móvil que va ENCIMA del dato por canal.\n"
                    "# 1 = ni gris ni mediana: las seis curvas crudas y nada más.\n"
                    "SUAVIZADO_CH = 15\n"
                    "try:\n"
                    "    import matplotlib.pyplot as plt\n"
