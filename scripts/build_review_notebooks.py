...
STAGES: list[dict] = [
    ...
    dict(
        ...
        plot_md=(
            ...
            "continuo → R < 1.\n"
            "- **Centro:** la zona de cielo usada (azul) sobre la luz-blanca, **con los ejes en "
            "píxeles** y el radio en arcsec marcado desde la primaria; el halo AO queda excluido.\n"
            "- **Derecha:** la **mediana del cielo por spaxel** vs λ — el nivel que queda *después* "
            "de que el DRS restara el cielo. No es cero: si sale sistemáticamente negativo, el DRS "
            "sobre-restó, y ese suelo es el que hay que quitar antes de integrar flujo en aperturas "
            "grandes (es exactamente lo que mide `scripts/measure_growth_curve.py` para `apcorr`).\n\n"
            "### Cómo se decide dónde está el cielo\n\n"
            "El criterio de la etapa es `build_source_mask` "
            "(`musepipe/reduction/sky_zap.py`), y define **fuente**, no cielo — el cielo es lo que "
            "sobra:\n\n"
            "1. **Umbral de brillo.** Sobre la luz-blanca: `fondo = mediana` de todo el campo, "
            "`σ = robust_sigma`, y se marca como fuente todo lo que supere `fondo + 3σ` "
            "(`threshold_sigma`).\n"
            "2. **Dilatación** de 2 px (`dilation_px`), para no dejar el borde de cada fuente "
            "justo fuera de la máscara.\n"
            "3. **Discos forzados** en las posiciones declaradas (`SourceRegion`). Con "
            "`auto_halo=True` el radio no es fijo: `estimate_halo_radius` mide el perfil radial y "
            "crece hasta que el brillo cae a `fondo + 1σ` medido en `r > 0.65·r_max`, más un margen "
            "de 5 px. Así el disco sigue al halo AO en vez de suponerlo.\n"
            "4. **Cielo = spaxels finitos NO enmascarados.** `sky_fraction` "
            "({{qc:stages/stage00s_qc.json:decision.sky_fraction}}) es esa fracción, y la etapa "
            "exige ≥ 0.25: por debajo no hay cielo suficiente para que ZAP aprenda nada.\n\n"
            "Es deliberadamente **conservador**: prefiere llamar fuente a un spaxel dudoso antes que "
            "meter halo en el cielo, porque contaminar el cielo sesga todo lo que venga después.\n\n"
            "> Necesita el kernel **MUSE** (astropy) y el cubo en disco. Usa una máscara de cielo "
            "aproximada (percentil 30 de flujo) en vez de la de `build_source_mask`, así que el R "
            "reproducido aquí difiere levemente del oficial "
            "({{qc:stages/stage00q_qc.json:m4_sky.R}}, máscara de A4); lo que importa es si cae del "
            "mismo lado del umbral `R ≤ 1.5`."
        ),
        plot_code=(
