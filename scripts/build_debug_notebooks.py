...
INLINE_SOURCES = {
    ...
    "C3": [
        ...
        ("musepipe/extraction/optimal.py", [
            ...
            # `fit_primary_psf_model_cube` delega en `fit_primary_psf_amplitudes`,
            # que es la misma matematica devolviendo ademas los vectores por canal
            # (lo que C1b necesita para restar por exposicion).
            "fit_primary_psf_amplitudes", "fit_primary_psf_model_cube",
        ]),
        # La rama que decide DE QUE CUBO sale `psfsub`. Sin ella el notebook
        # restaba siempre un modelo del apilado, que es lo que la etapa hacia
        # ANTES de C1b: reproducia `ls` al 100 % y `psfsub` al 0 %.
        ("musepipe/stages/stage_x02_optimal.py", ["_load_perobs_psfsub_cube"]),
    ],
    ...
    "C1": [
        ...
        # La §10 ajusta los cubos POR EXPOSICION, que no traen extension
        # WAVELENGTH: su eje lambda sale de la cabecera, con la misma funcion que
        # usa la cadena (y que D2 copia por lo mismo). `TelluricError` viaja
        # porque es la que levanta cuando no puede reconstruirlo.
        ...
        ("musepipe/psf.py", [
            ...
            # La forma `mixture` (la PSF del combinado como media pesada de las de
            # cada exposicion) entra por `evaluate_psf_model`: si no viajara, la
...
def build_a3_cells(mb, target, run_id):
    ...
    cells = [
        ...
        code(
            ...
            "        # `clip_negative=False` por partida doble: A3 publica las profundidades\n"
            "        # CON SIGNO (telluric.py:1309), y `decide_telluric` compara desde la\n"
            "        # puerta de dos colas el MAXIMO EN VALOR ABSOLUTO — asi que recortar\n"
            "        # aqui falseaba tambien el veredicto, no solo las cifras.\n"
            ...
            "    w, antes, despues = par\n"
            # El caso real: la reduccion ADP de julio del primer objeto trae tres
            # bandas con `H2O_8200: 0.0` y ningun `spec_version`.
            "    # Un QC anterior a A3 v3 publico sus profundidades RECORTADAS a cero, asi\n"
            "    # que compararlas contra las de hoy —que llevan signo— es comparar dos\n"
            "    # convenciones, no dos numeros: antes coincidian porque ambos lados\n"
            "    # recortaban, y eso escondia que ese QC esta sin re-correr.\n"
            "    if str((r['qc'] or {}).get('spec_version') or '') != 'A3_v3':\n"
            "        print('    QC anterior a A3 v3 (profundidades recortadas): no se compara')\n"
            "        continue\n"
            "    # `clip_negative=False`: A3 publica las profundidades CON SIGNO desde la\n"
            "    # puerta de dos colas (telluric.py:1309). Compararlas contra la version\n"
            "    # recortada daba 0.00 frente a -1.00 y ponia el notebook en DIFIERE sin\n"
            "    # que ningun numero estuviera mal.\n"
            "    mias = measure_telluric_depths(w, antes, bands=BANDS_A, clip_negative=False)\n"
            "    for banda, suyo in r['profundidades'].items():\n"
            ...
            "        post = measure_telluric_depths(w, despues, bands=BANDS_A, clip_negative=False)\n"
...
def build_c2_cells(mb, target, run_id):
    ...
    cells = [
        ...
        code(
            ...
            "# Suavizado de PRESENTACIÓN, apagado por defecto: lo que se dibuja es el dato,\n"
            "# canal a canal. Iba fijo a 41 canales y no se podía apagar, que es justo lo\n"
            "# que hace que un espectro parezca mejor de lo que es. Súbelo para ver la\n"
            "# tendencia: no cambia ningún número, solo la línea. Si ya lo pusiste en otra\n"
            "# celda, esa elección manda.\n"
            "SUAVIZADO_CH = globals().get('SUAVIZADO_CH', 1)\n"
            "suave = (lambda v: median_filter_1d(np.asarray(v, dtype=float), SUAVIZADO_CH)\n"
            "         if SUAVIZADO_CH > 1 else np.asarray(v, dtype=float))\n"
            "_ETIQ_SUAVE = f'mediana móvil {SUAVIZADO_CH} ch' if SUAVIZADO_CH > 1 else 'por canal'\n\n"
            ...
            "a1.plot(WAVE, suave(ref_flux), lw=1.6, color='0.6', label='cadena')\n"
            "a1.plot(WAVE, suave(flux), lw=1.0, color='tab:blue', ls='--', label='este notebook')\n"
            "a1.legend(fontsize=8); a1.set_ylabel(f'flujo ({_ETIQ_SUAVE})')\n"
...
def build_c3_cells(mb, target, run_id):
    ...
    return [
        md(
            ...
            "| `optimal_psfsub` | el cubo con la **primaria restada**: el de C1b si existe "
            "(restada en cada exposición), y si no, el de B2 menos un modelo ajustado aquí canal a "
            "canal | anticipa el fondo de C4; `ls` vs `psfsub` es el diagnóstico del "
        ...
        code(
            ...
            "# De donde sale el cubo de `psfsub`. La etapa mira la perilla Y si el\n"
            "# fichero existe: si C1b dejo su cubo, la primaria ya se resto en CADA\n"
            "# exposicion y aqui no se resta nada (`stage_x02_optimal` §6).\n"
            "PSFSUB_PEROBS_ON   = bool(X02.get('x02_psfsub_per_observation', True))\n"
            "PSFSUB_CUBE_PATH   = Path(X02.get('x02_psfsub_cube_fits') or (SD / 'cube_psfsub_perobs.fits'))\n"
            "USA_PEROBS         = PSFSUB_PEROBS_ON and PSFSUB_CUBE_PATH.exists()\n"
            ...
            "                      'radio exclusión compañero': PRIMARY_EXCL_RADIUS,\n"
            "                      'cubo de psfsub': ('C1b, restado por exposición' if USA_PEROBS\n"
            "                                        else 'B2 menos un modelo del apilado')}.items()):\n"
            "    print(f'  {_k:26s} {_v}')"
        ),
        md(
            "## 2 · Entradas — **los dos cubos**\n\n"
            "`ls` sale del residual de 04b; `psfsub`, del cubo con la primaria ya restada (§6). "
            "Todos deben tener la misma forma y el mismo eje λ: la cadena lo exige y para aquí si "
            "no (serían dos rejillas distintas)."
        ),
        code(
            "qc_b3 = json.loads((SD / 'stage01c_qc.json').read_text(encoding='utf-8'))\n"
            "OBJECT_YX = tuple(float(v) for v in qc_b3['companion']['pos_yx'])\n"
        ...
        md(
            ...
            "**Pero desde C1b hay dos caminos, y la etapa elige uno.** Si C1b dejó su cubo "
            "(`cube_psfsub_perobs.fits`), la primaria se ajustó y se restó en **cada exposición** "
            "con SU modelo y los residuos se combinaron después; entonces C3 no resta nada, solo "
            "consume ese cubo. Restar aquí obliga a describir con **un** modelo la mezcla de 29 "
            "PSF distintas, que es exactamente lo que el ajuste por observación existe para "
            "evitar.\n\n"
            "La celda hace **las dos cosas**: ajusta el modelo único (es el objeto de estudio de "
            "esta sección, y la referencia) pero extrae del cubo que consume la etapa, tomando su "
            "misma rama. La diferencia entre los dos es, literalmente, lo que aporta C1b.\n\n"
            "*(Es la celda cara: ajusta un modelo por canal. Un par de minutos.)*"
        ),
        code(
            "# El ajuste se hace SIEMPRE, porque es el objeto de estudio de esta\n"
            "# seccion: es lo que la etapa hacia antes de C1b y sigue siendo la\n"
            "# referencia contra la que se mira la resta por exposicion.\n"
            ...
            "PSFSUB_UN_MODELO = STAGE02 - primary_model\n"
            "print('ajuste de la primaria:', {kk: psfsub_meta[kk] for kk in list(psfsub_meta)[:4]})\n\n"
            "# Pero el cubo que se EXTRAE es el que consume la etapa. La rama es la\n"
            "# suya, copiada, no una reimplementacion: si C1b dejo su cubo, `psfsub`\n"
            "# sale de ahi y este ajuste solo sirve de comparacion.\n"
            "if USA_PEROBS:\n"
            "    PSFSUB_CUBE, _w_perobs, _bunit_perobs = _load_perobs_psfsub_cube(\n"
            "        PSFSUB_CUBE_PATH, expected_shape=STAGE02.shape, expected_wave=WAVE)\n"
            "    _d = PSFSUB_CUBE - PSFSUB_UN_MODELO\n"
            "    _fin = np.isfinite(_d)\n"
            "    print(f'cubo de psfsub: C1b ({PSFSUB_CUBE_PATH.name}) — la primaria se resto en\\n'\n"
            "          f'                CADA exposicion y los residuos se combinaron despues.')\n"
            "    print(f'  frente a restar UN modelo del apilado: mediana |Δ| = '\n"
            "          f'{np.nanmedian(np.abs(_d[_fin])):.4g}, max |Δ| = {np.nanmax(np.abs(_d[_fin])):.4g}')\n"
            "    print('  esa diferencia es TODO lo que aporta C1b, y es lo que esta seccion compara.')\n"
            "else:\n"
            "    PSFSUB_CUBE = PSFSUB_UN_MODELO\n"
            "    print('cubo de psfsub: B2 menos el modelo de arriba (C1b no ha dejado cubo en este run).')\n\n"
            ...
            "# Con C1b el tercer panel NO es `dato - modelo`: es el residuo que dejo\n"
            "# la resta por exposicion. Decirlo, porque si no la figura se lee como\n"
            "# una resta que no es la que se hizo.\n"
            "axes[2].set_title(('psfsub = residuo de C1b (por exposición)' if USA_PEROBS\n"
            "                   else 'residual = psfsub = B2 − modelo') + '  ·  LINEAL simétrica',\n"
            "                  fontsize=9)\n"
        ...
        code(
            ...
            "    ax.plot(WAVE, suave(opt['flux']), lw=1.3, color='tab:blue',\n"
            "            label=f'ventana óptima r={float(WINDOW_RADIUS_PX):g} px')\n"
            "    ax.plot(WAVE, suave(ap['flux']), lw=1.3, color='tab:orange',\n"
        ...
        code(
            ...
            "                             gridspec_kw={'height_ratios': [2, 1]})\n"
            "a1.plot(WAVE, suave(LS['flux']), lw=1.1, label='optimal_ls')\n"
            "a1.plot(WAVE, suave(PSFSUB['flux']), lw=1.1, label='optimal_psfsub')\n"
            "a1.axhline(0, color='0.7', lw=0.6); a1.axvline(6563, color='tab:red', ls=':')\n"
            "a1.legend(fontsize=8); a1.set_ylabel(f'flujo ({_ETIQ_SUAVE})')\n"
            "a2.plot(WAVE, suave(LS['flux'] - PSFSUB['flux']), lw=1.0, color='tab:purple')\n"
            "a2.axhline(0, color='0.7', lw=0.6)\n"
        ...
        code(
            ...
            "    ax.plot(WAVE, suave(v['flux']), lw=1.4, color='tab:blue',\n"
            "            label='compañero')\n"
            ...
            "        ax.plot(WAVE, suave(f_c), lw=0.9, alpha=0.8, label=etiqueta)\n"
        ...
        code(
            ...
            '    ejes[0].plot(WAVE, suave(crudo), lw=1.2, color=color, label=nombre)\n'
            '    if ctrl_med is not None:\n'
            "        ejes[0].plot(WAVE, suave(ctrl_med), lw=1.0, ls='--', color=color,\n"
            "                     alpha=0.7, label=nombre + ' · mediana de los controles')\n"
            "    ejes[1].plot(WAVE, suave(v['flux']), lw=1.2, color=color, label=nombre)\n"
            '    if ctrl_med is not None:\n'
            "        ejes[1].plot(WAVE, suave(ctrl_med * v['apcorr']), lw=1.0, ls='--',\n"
        ...
        code(
            ...
            "    ax.plot(WAVE, suave(np.asarray(ref.flux, float)), lw=1.6,\n"
            "            color='0.6', label='cadena')\n"
            "    ax.plot(WAVE, suave(mio['flux']), lw=1.0, ls='--',\n"
...
def build_c4_cells(mb, target, run_id):
    ...
    return [
        ...
        md(
            ...
            "### Sobre el suavizado\n\n"
            "**No hay**: `SUAVIZADO_CH = 1` por defecto, así que lo que se dibuja es el dato, "
            "canal a canal. Antes iba fija a 11 y no se decía, que es justo lo que hace que un "
            "espectro parezca mejor de lo que es. Súbela si quieres ver la tendencia; no cambia "
            "ningún número, solo la línea, y la etiqueta de la figura dice cuál está puesta.\\n\\n"
        ...
        code(
            "from musepipe.spectral import median_filter_1d\n\n"
            "SUAVIZADO_CH = 1         # 1 = sin suavizar: lo que se ve es el dato\n"
            ...
            "a1.set_title('la primaria por dos caminos independientes'\n"
            "             + (f'  (líneas: mediana móvil de {SUAVIZADO_CH} ch; gris: por canal)'\n"
            "                if SUAVIZADO_CH > 1 else '  (sin suavizar: una línea por canal)'),\n"
            "             fontsize=9)\n"
...
def build_halosub_cells(mb, target, run_id, stage_id):
    ...
    return [
        ...
        code(
            ...
            "                             gridspec_kw={'height_ratios': [2, 1]})\n"
            "a1.plot(WAVE, suave(ref_flux), lw=1.6, color='0.6', label='cadena')\n"
            "a1.plot(WAVE, suave(flux), lw=1.0, ls='--', color='tab:blue',\n"
            "        label='este notebook')\n"
            "a1.legend(fontsize=8); a1.set_ylabel(f'flujo ({_ETIQ_SUAVE})')\n"
            "a2.plot(WAVE, flux - ref_flux, lw=0.7, color='tab:purple')\n"
            "a2.axhline(0, color='0.7', lw=0.6)\n"
...
def build_d2_star_cells(mb, target, run_id):
    ...
    cells = [
        ...
        code(
            ...
            "                                 gridspec_kw={'height_ratios': [2, 1]})\n"
            "    a1.plot(CADENA.wave_A, suave(np.asarray(CADENA.flux, float)),\n"
            "            lw=1.6, color='0.6', label='cadena')\n"
            "    a1.plot(WAVE, suave(FLUJO), lw=1.0, color='tab:blue', ls='--',\n"
            "            label='este notebook')\n"
            "    a1.legend(fontsize=8); a1.set_ylabel(f'flujo ({_ETIQ_SUAVE})', fontsize=8)\n"
        ...
        code(
            ...
            "        print('grado 2, que a 8 bandas de una curva que cae deprisa y luego se aplana')\n"
            "        print('le pone un vértice interior. El tramo rojo del continuo sale')\n"
            "        print('sobre-corregido en LOS SEIS métodos del bloque C.')\n"
            "        print('YA ESTÁ ARREGLADO en la cadena (2026-08-11): `factor_at_wavelengths`')\n"
            "        print('interpola con `pchip`. Que esto salte quiere decir que ESTE PRODUCTO es')\n"
            "        print('anterior al cambio — la §10.b lo confirma y mide cuánto se movería.')\n"
            "    if _pp >= 1.10:\n"
        ...
        md(
            "### 10.b · Los dos interpoladores de la apcorr, uno contra otro\n\n"
            "La sección anterior mide la apcorr **tal como viene en el producto**. Esta compara "
            "las **dos formas de construirla** a partir de las mismas 8 bandas, porque el "
            "2026-08-11 cambió la de por defecto:\n\n"
            "* **`poly2`** — el ajuste polinomial de grado 2, histórico. Una parábola tiene "
            "curvatura **constante**: no puede bajar deprisa y luego aplanarse, que es exactamente "
            "lo que hace esta curva. Al forzarla, le pone el **vértice dentro del rango** y la "
            "hace repuntar hacia el rojo — un repunte que **no está en las bandas**, lo fabrica el "
            "ajuste. Y de paso no pasa por los puntos medidos.\n"
            "* **`pchip`** — cúbica **monótona a trozos**. Pasa *por* las bandas (residuo cero por "
            "construcción) y no inventa curvatura entre ellas: si el dato baja, baja. Fuera del "
            "rango de bandas extrapola **en recta** con la pendiente del extremo, porque la cúbica "
            "extrapolada se dispara y aquí se extrapola de verdad (el eje llega a 4750 Å y la "
            "banda más azul está en 5037).\n\n"
            "**Por qué importa y no es cosmético.** La apcorr **multiplica**, y es una curva "
            "**común**: entra idéntica en los seis métodos del bloque C. Un error en su forma no "
            "aparece como dispersión entre métodos —que es donde D1 miraría— sino como un sesgo "
            "solidario en la **pendiente del continuo**. Por eso hay que verlo aquí.\n\n"
            "La celda además dice **cuál de las dos lleva el producto que hay en disco**. Si lleva "
            "`poly2`, es de antes del cambio y el bloque C está sin re-correr: la comparación de "
            "abajo es entonces la medida de lo que se movería al re-correrlo, no de lo que ya "
            "está aplicado."
        ),
        code(
            "# Los cuatro se re-importan aquí a propósito: los de la celda de arriba\n"
            "# viven DENTRO de su `else`, y si este run no tiene cubo de entrada esa\n"
            "# rama no corre y esta celda moriría con un NameError.\n"
            "from musepipe.growth_curve import (factor_at_wavelengths, interior_bump,\n"
            "                                   interior_dip, MAX_INTERIOR_BUMP)\n\n"
            "_gcb = nb.load_qc_optional('stages/growth_curve_qc.json', RUN_ID)\n"
            "if not _gcb or not _gcb.get('bands'):\n"
            "    print('este run no tiene `growth_curve_qc.json`: no hay apcorr empírica que'\n"
            "          ' comparar (convención `normrad`).')\n"
            "else:\n"
            "    _xb = np.array([b['wave_A'] for b in _gcb['bands']], dtype=float)\n"
            "    _yb = np.array([b['ratio_total_over_normrad'] for b in _gcb['bands']], dtype=float)\n"
            "    _o = np.argsort(_xb); _xb, _yb = _xb[_o], _yb[_o]\n"
            "    # `require_monotonic=False` en las DOS: aquí se dibuja para diagnosticar,\n"
            "    # y la gracia es poder ver la curva que el guardia rechaza.\n"
            "    CURVAS = {m: np.asarray(factor_at_wavelengths(_gcb, WAVE, method=m,\n"
            "                                                 require_monotonic=False), float)\n"
            "              for m in ('poly2', 'pchip')}\n"
            "    print(f'{len(_xb)} bandas medidas, de {_xb[0]:.0f} a {_xb[-1]:.0f} Å'\n"
            "          f'  ({_yb[0]:.4f} -> {_yb[-1]:.4f}, cae un'\n"
            "          f' {100 * (1 - _yb[-1] / _yb[0]):.1f}%)')\n"
            "    _sube = 100 * (_yb[-1] / _yb[-2] - 1)\n"
            "    print(f'el último tramo medido {_xb[-2]:.0f}->{_xb[-1]:.0f} Å'\n"
            "          f' {\"sube\" if _sube > 0 else \"baja\"} un {abs(_sube):.2f}%'\n"
            "          '  <- esto es lo único que el dato dice del repunte\\n')\n"
            "    print(f'{\"método\":>8s}{\"joroba\":>9s}{\"repunte\":>9s}{\"|Δ| vs bandas\":>15s}'\n"
            "          f'{\"mínimo en\":>12s}{\"extremos\":>18s}')\n"
            "    for _m, _c in CURVAS.items():\n"
            "        _enb = np.asarray(factor_at_wavelengths(_gcb, _xb, method=_m,\n"
            "                                               require_monotonic=False), float)\n"
            "        _i = int(np.nanargmin(_c))\n"
            "        _min = f'{WAVE[_i]:.0f} Å' if 0 < _i < WAVE.size - 1 else 'en el borde'\n"
            "        print(f'{_m:>8s}{100 * interior_bump(_c):8.2f}%{100 * interior_dip(_c):8.2f}%'\n"
            "              f'{100 * np.max(np.abs(_enb / _yb - 1)):14.2f}%{_min:>12s}'\n"
            "              f'{_c[0]:9.4f} ..{_c[-1]:8.4f}')\n"
            "    print(f'\\numbral del guardia: joroba y repunte por encima de'\n"
            "          f' {100 * MAX_INTERIOR_BUMP:.0f}% paran la etapa.')\n\n"
            "    # ¿Cuál lleva el producto que hay en disco? C4 no tiene apertura (su\n"
            "    # apcorr nominal es 1), así que lo que viaja en `apcorr` ES este factor.\n"
            "    _apc = globals().get('APC')\n"
            "    if _apc is not None:\n"
            "        _cual = {m: float(np.nanmax(np.abs(_apc / c - 1))) for m, c in CURVAS.items()}\n"
            "        _lleva = min(_cual, key=_cual.get)\n"
            "        print(f'\\nel producto en disco lleva `{_lleva}`'\n"
            "              f'  (|Δ| máx {100 * _cual[_lleva]:.3f}%, contra'\n"
            "              f' {100 * max(_cual.values()):.2f}% del otro)')\n"
            "        if _lleva != 'pchip':\n"
            "            print('  -> es ANTERIOR al cambio del 2026-08-11: el bloque C está sin'\n"
            "                  ' re-correr,')\n"
            "            print('     y lo de abajo es lo que se movería al re-correrlo.')\n\n"
            "    _r = CURVAS['pchip'] / CURVAS['poly2'] - 1.0\n"
            "    print('\\ndiferencia pchip/poly2 a lo largo del eje:')\n"
            "    for _lam in (WAVE[0], 5000.0, 6563.0, 8000.0, 9000.0, WAVE[-1]):\n"
            "        _i = int(np.argmin(np.abs(WAVE - _lam)))\n"
            "        _nota = '  <- Hα' if abs(_lam - 6563.0) < 1 else ''\n"
            "        _nota += '  (extrapolado)' if (_lam < _xb[0] or _lam > _xb[-1]) else ''\n"
            "        print(f'   {WAVE[_i]:7.0f} Å   {100 * _r[_i]:+6.2f}%{_nota}')\n"
            "    _incl = 100 * ((1 + _r[-1]) / (1 + _r[0]) - 1)\n"
            "    print(f'\\nINCLINACIÓN del continuo de punta a punta: {_incl:+.2f}%'\n"
            "          '  (el azul sube y el rojo baja respecto a poly2)')\n"
            "    print('Se aplica IGUAL a los seis métodos del bloque C, así que no aparece')\n"
            "    print('como dispersión inter-método: es un sesgo común en la pendiente.')\n\n"
            "    fig, (b1, b2) = plt.subplots(2, 1, figsize=(11.5, 6), sharex=True,\n"
            "                                 gridspec_kw={'height_ratios': [1.5, 1]})\n"
            "    b1.plot(WAVE, CURVAS['poly2'], lw=1.4, color='tab:red',\n"
            "            label='poly2 (histórico): vértice interior')\n"
            "    b1.plot(WAVE, CURVAS['pchip'], lw=1.4, color='tab:blue',\n"
            "            label='pchip (por defecto): pasa por las bandas')\n"
            "    b1.plot(_xb, _yb, 'ko', ms=5, zorder=5, label='las 8 bandas medidas')\n"
            "    b1.axvspan(WAVE[0], _xb[0], color='0.9', zorder=0)\n"
            "    b1.axvspan(_xb[-1], WAVE[-1], color='0.9', zorder=0)\n"
            "    b1.set_ylabel('factor normrad -> total', fontsize=8)\n"
            "    b1.set_title('en gris, fuera de las bandas: ahí las dos extrapolan', fontsize=9)\n"
            "    b1.legend(fontsize=8)\n"
            "    b2.plot(WAVE, 100 * _r, lw=1.0, color='tab:purple')\n"
            "    b2.axhline(0.0, color='0.6', lw=0.8, ls=':')\n"
            "    b2.set_ylabel('pchip / poly2 − 1 [%]', fontsize=8)\n"
            "    b2.set_xlabel('λ [Å]', fontsize=8)\n"
            "    fig.tight_layout(); plt.show()"
        ),
        md(
...
def build_c1_cells(mb, target, run_id):
    ...
    que usan todas las demas**. Por eso la comparacion de la §12 no es contra un
    ...
    return [
        md(
            ...
            "2. **El ensamblado y la evaluación del modelo** (§8, §12) son baratos y se hacen "
        ...
        code(
            ...
            "# Los defaults del esquema de pesos vienen de `musepipe`, no copiados aqui:\n"
            "# esta celda corre ANTES que la copia editable de la §3.\n"
            "from musepipe.stages.stage_e01_psfao import (\n"
            "    PSFAO_DEFAULT_WEIGHT_CAP, PSFAO_DEFAULT_WEIGHTING)\n\n"
            "# `project_root=ROOT`: musepipe resuelve rutas contra el cwd, que en un\n"
            "# notebook es su propia carpeta, no la raíz del repo.\n"
            ...
            "# Que parte de la imagen decide el ajuste. Se lee de la etapa, NO se supone:\n"
            "# la §10 ajusta cubo a cubo para contrastar contra el modelo de la cadena, y\n"
            "# si ajustara con otros pesos estaria comparando peras con manzanas.\n"
            "PESOS_AJUSTE     = str(E01.get('psf_fit_weighting', PSFAO_DEFAULT_WEIGHTING))\n"
            "TOPE_PESOS       = E01.get('psf_fit_weight_cap', PSFAO_DEFAULT_WEIGHT_CAP)\n"
            ...
            "# Con `True` (el default) la §10 SOLO usa lo que ya esté en la caché: si no\n"
            "# hay rejilla para esta clave, dice lo que costaría y se salta. La clave\n"
            "# incluye los pesos del ajuste y la matemática copiada, así que tocar C1 la\n"
            "# invalida — y sin esta perilla el test `slow` pagaba ~50 min cada vez.\n"
            "# Ponlo en `False` cuando lo que quieras auditar sea justamente esta sección.\n"
            "MAPA_SOLO_SI_CACHE = True\n"
        ...
        md(
            ...
            "`stages/` describe una PSF que ya no es la de estos datos, y las secciones §8–§12 "
        ...
        md(
            "### Qué es cada parámetro, y qué es cada color\n\n"
            "Psfao (Fétick et al. 2019, A&A 628, A99 — lo que implementa `maoppy.psfmodel.Psfao`) "
            "**no parametriza la imagen de la PSF**, sino la densidad espectral de potencia (PSD) "
            "de la fase del frente de onda. La imagen sale de ahí en tres pasos, y los siete "
            "números de estos paneles entran todos en el primero:\n\n"
            "$$W(f)\\;=\\;\\underbrace{0.0229\\;r_0^{-5/3}\\left(L_{\\rm ext}^{-2}+f^2\\right)^{-11/6}"
            "\\,\\bigl(1-\\Pi\\bigr)}_{\\text{halo von Kármán: lo que la AO no corrige}}"
            "\\;+\\;\\underbrace{\\Pi\\,\\bigl[\\,C\\;+\\;A\\,M_\\beta(f)\\,\\bigr]}"
            "_{\\text{dentro del área corregida}}$$\n\n"
            "$$M_\\beta(f)\\;=\\;E\\,\\bigl(1+u\\bigr)^{-\\beta},\\qquad "
            "u=\\frac{f_x'^2}{\\alpha_x^2}+\\frac{f_y'^2}{\\alpha_y^2},\\qquad "
            "\\alpha_x=\\alpha\\cdot\\mathrm{ratio},\\quad \\alpha_y=\\alpha/\\mathrm{ratio}$$\n\n"
            "donde $\\Pi$ vale 1 dentro del área que corrige la AO ($f<f_{\\rm AO}=N_{\\rm act}/2D$) "
            "y 0 fuera, $(f_x',f_y')$ son las frecuencias giradas un ángulo $\\theta$, "
            "$L_{\\rm ext}=10$ m es la escala externa de von Kármán, $E$ normaliza la energía del "
            "Moffat dentro de $f_{\\rm AO}$, y $W(0)=0$.\n\n"
            "De la PSD a la imagen: $B_\\phi=\\mathcal{F}[W]$ da la covarianza de la fase, "
            "$D_\\phi(\\rho)=2\\left[\\int W-B_\\phi(\\rho)\\right]$ la función de estructura, y de ahí "
            "$\\mathrm{OTF}=e^{-D_\\phi/2}\\cdot\\mathrm{OTF}_{\\rm difracción}\\cdot"
            "\\mathrm{OTF}_{\\rm jitter}$, cuya transformada inversa es la PSF. Todo el cromatismo "
            "que ve la cadena entra por aquí: los tres primeros parámetros escalan con λ "
            "($r_0\\propto\\lambda^{6/5}$, la ley de Fried, y $C,A\\propto\\lambda^{-2}$) y los "
            "otros cuatro son acromáticos por construcción.\n\n"
            "| # | parámetro | unidad | qué es | dónde se ve en la PSF |\n"
            "|---|---|---|---|---|\n"
            "| 0 | `r0` | m | parámetro de Fried: la fuerza de la turbulencia que **queda sin "
            "corregir** | la amplitud del halo ancho, $\\propto r_0^{-5/3}$. Más grande = mejor "
            "seeing = halo más débil |\n"
            "| 1 | `C` | rad² m² | suelo plano de la PSD dentro del área corregida (aliasing + "
            "ruido del sensor de frente de onda) | levanta por igual toda la zona corregida: la "
            "meseta bajo el núcleo |\n"
            "| 2 | `A` | rad² | varianza de la fase residual en el área corregida — la potencia "
            "total del término Moffat | el **halo AO**, el anillo que muere en el borde de la "
            "corrección. Es el que más manda en cuánta luz cae al radio del compañero |\n"
            "| 3 | `alpha` | 1/m | anchura del Moffat **en frecuencia** | `alpha` pequeño "
            "concentra la potencia en frecuencias bajas = estructura ancha en la imagen |\n"
            "| 4 | `ratio` | — | elongación, $\\sqrt{\\alpha_x/\\alpha_y}$ | 1 = residuo circular; "
            "distinto de 1 = alargado |\n"
            "| 5 | `theta` | rad | ángulo de esa elongación | orienta el alargamiento |\n"
            "| 6 | `beta` | — | índice de la ley de potencias del Moffat | con qué rapidez cae el "
            "residuo con la frecuencia: gobierna las alas del núcleo corregido |\n\n"
            "**Cuidado al leer `ratio` y `theta`**: $(\\mathrm{ratio},\\theta)$ y "
            "$(1/\\mathrm{ratio},\\theta+\\pi/2)$ describen **la misma PSF**. Los puntos negros "
            "pueden saltar de una rama a la otra sin que cambie nada físico — medido al rehacer "
            "los bins de 8100–8600 Å, donde los dos arranques dieron `ratio` 0.829 y 1.206 = "
            "1/0.829 con coste, anillo y apcorr idénticos. Esa degeneración es justo la razón de "
            "que estos siete números **no se puedan suavizar uno a uno** (§12).\n\n"
            "**Los colores de los paneles:**\n\n"
            "- **Puntos negros** — los nodos de `param_table`: los bins que C1 ajustó de verdad. "
            "Entre dos puntos negros no hay ninguna medida.\n"
            "- **Línea azul** — lo que la cadena evalúa de verdad: la interpolación **lineal** de "
            "esos nodos, en `w_eff` (la λ redondeada a `psfao_wave_bin_A`). Es la única curva que "
            "acaba en un espectro.\n"
            "- **Línea naranja discontinua** — `smoothed_poly`, la parábola por parámetro que el "
            "QC anuncia. Mientras exista `param_table` **no se evalúa nunca**; se dibuja para que "
            "se vea cuánto se aparta de lo que sí se usa.\n"
            "- **Bandas grises** — huecos de `param_table` (más de un bin seguido sin ajuste), "
            "donde esa línea azul es una recta larga sin dato debajo. La **banda roja** es el "
            "hueco más ancho.\n\n"
            "Si el objeto salió `moffat`, los paneles son sus seis parámetros de forma y la "
            "lectura cambia: ahí **la línea azul ya es el polinomio suavizado** —que en esa forma "
            "sí es lo que se evalúa, en la λ exacta y sin redondeo— y no hay curva naranja ni "
            "huecos que marcar."
        ),
        code(
        ...
        md(
            ...
            "Hasta aquí todo mira **un** cubo: el combinado sobre el que ajusta la cadena. Esta "
            "sección abre la otra dimensión, la que nadie ha mirado nunca — **la exposición** — "
            "y lo hace para las **dos formas de PSF a la vez**.\n\n"
            "> **Aquí se ajustan Moffat y Psfao siempre**, gane quien gane. Es a propósito y es "
            "lo contrario que el resto del notebook, que audita solo la forma entregada: la "
            "comparación entre las dos *es* el objetivo de esta sección.\n\n"
            "**Por qué importa, y no es curiosidad.** La §8 dejó los escalones atribuidos a la "
            "degeneración de `C`/`alpha`, al recorte a los límites físicos y a la cuantización "
            "de λ a 50 Å. Lo que ninguna de esas medidas puede decidir es **cuánto de la "
            "estructura en λ es física y cuánto es ruido del ajuste**, porque con un solo cubo "
            "no hay con qué comparar. La dispersión entre exposiciones sí lo decide:\n\n"
            "* si a λ fija los cubos se dispersan **tanto como** saltan los bins contiguos, los "
            "escalones son ruido y suavizar en λ está justificado;\n"
            "* si la estructura en λ **se repite** en todas las exposiciones, es cromática y de "
            "verdad, y suavizarla sería borrar señal.\n\n"
            "La §10.d lo mide y lo dice con número.\n\n"
            "**Qué es «un cubo» aquí.** Los cubos **por exposición** que el config declara en "
            "`perexp_cubes` / `perexp_dir`, nunca por convención de nombres sobre un disco. Un "
            "objeto que no los declare se salta la sección entera; para tenerlos, "
        ...
        md(
            ...
            "Un cubo por exposición **no es** el stack de B2: es un `DATACUBE_FINAL` en su propio "
            "marco de píxeles, más grande y con la estrella donde le toque. Tres ajustes de "
            "encuadre, ninguno de física:\n\n"
            "1. **`fit_bin` supone la primaria en el centro del array** (`cy, cx = ny//2, nx//2`). "
            "En el cubo de la cadena eso se cumple porque B1 recorta con la primaria en el "
            "centro. Así que de cada exposición se recorta una ventana **del mismo tamaño** "
            "centrada en su propia primaria, localizada por máximo + centroide de la imagen "
            "blanca — la misma receta que usa D2. El centrado es a píxel entero (≤ 0.5 px de "
            "resto), y ese resto lo absorbe el `dxdy` del propio ajuste, igual que en la cadena.\n"
            "2. **Dónde cae el compañero.** Se toma su desplazamiento respecto del **centro del "
            "array** en el cubo de la cadena —que es lo que `fit_bin` usa de verdad para "
            "enmascarar y para medir el anillo— y se aplica igual en cada ventana. Vale porque "
            "el DRS remuestrea todas las exposiciones a la misma orientación: `CD1_2 = 0` en "
            "todas, aunque el rotador (`ADA POSANG`) se mueva entre ellas.\n"
            "3. **Bins y radios** son los de la cadena, del config resuelto (`E01`), no "
            "literales. Los bins se recalculan sobre el eje λ **de cada cubo**.\n\n"
            "Se lee solo lo necesario: los canales de los bins elegidos, ya recortados a la "
            "ventana. `STAT` está en todas las exposiciones, así que los pesos del ajuste son "
            "los mismos que usa la cadena.\n\n"
            "> **La caché.** Esto cuesta ~4.5 s por ajuste de Psfao (0.3 s los de Moffat), así "
            "que el resultado se guarda en `runs/<RUN>/tables/c1_perexp_psf_params.json` y las "
            "siguientes ejecuciones lo leen en menos de un segundo. La clave de la caché "
            "incluye las rutas y fechas de los cubos, los bins, los radios, la posición del "
            "compañero y los `sha` de las funciones copiadas: si cambia cualquiera de esas "
            "cosas, se recalcula solo. Para forzarlo, `RECALCULAR_MAPA = True`; para tirarla, "
            "borra el fichero. **Es el único notebook `debug` que escribe en disco**, y no "
            "escribe nada si el run está marcado `legacy`."
        ),
        code(
            ...
            "        # Sin esto, cambiar `psf_fit_weighting` en el config reusaria una\n"
            "        # rejilla ajustada con otros pesos, y nadie lo veria: la clave solo\n"
            "        # se salva hoy de rebote, porque lleva los `shas` del codigo copiado.\n"
            "        'pesos': [str(PESOS_AJUSTE),\n"
            "                  None if TOPE_PESOS is None else float(TOPE_PESOS)],\n"
            ...
            "                               weighting=PESOS_AJUSTE, weight_cap=TOPE_PESOS)\n"
            "        for r in fp:\n"
            ...
            "              f' de {CACHE_MAPA}')\n"
            "    elif MAPA_SOLO_SI_CACHE and not RECALCULAR_MAPA:\n"
            "        # No hay rejilla para esta clave y nadie ha pedido pagarla. Se dice\n"
            "        # lo que costaría y se sigue: el resto del notebook no depende de\n"
            "        # esta sección, y sin esta salida el test `slow` pagaba ~50 min cada\n"
            "        # vez que se tocaba C1.\n"
            "        _nb = len(range(0, len(BINS_PSFAO), PASO_BINS_MAPA))\n"
            "        _falta = [e for e, _p in OBS_C1 if e not in GEOM_MAPA]\n"
            "        FILAS_MAPA = []\n"
            "        print(f'§10 SALTADA: no hay caché para esta clave ({_sha_clave}).')\n"
            "        print(f'   costaría {len(_falta)} cubos × ~{_nb} bins × 2 formas'\n"
            "              f' = ~{_nb * len(_falta)} ajustes Psfao a ~4.5 s (~'\n"
            "              f'{_nb * len(_falta) * 4.5 / 60:.0f} min).')\n"
            "        print('   para calcularla: MAPA_SOLO_SI_CACHE = False (o'\n"
            "              ' RECALCULAR_MAPA = True para rehacerla).')\n"
            "        print(f'   la clave incluye los pesos del ajuste ({PESOS_AJUSTE},'\n"
            "              f' tope {TOPE_PESOS}) y la matemática copiada, así que tocar C1'\n"
            "              ' la invalida.')\n"
            "    else:\n"
        ...
        md(
            ...
            "Cada parámetro contra λ, **con su barra de error**, para dos exposiciones. Por "
            "defecto la primera de cada noche (`CUBOS_LINEAS` lo cambia), porque el par "
            "interesante es el que cruza los dos OB. En gris, el ajuste del **cubo combinado** "
            "que hay en `stages/`, como referencia. Los ajustes que no convergen van con "
            "marcador hueco: un estancamiento tiene que verse, no desaparecer.\n\n"
            "> **Las barras de las dos formas NO significan lo mismo.** Psfao propaga "
            "`1/sqrt(diag(JᵀJ))` con pesos `1/STAT`, y `docs/noise_model.md` tiene medido que "
            "STAT subestima el ruido: es una **cota inferior formal**. Moffat usa "
            "`pinv(JᵀJ)·σ²` con σ **empírica** del residuo. Sirven para juzgar la estabilidad "
            "*dentro* de cada forma; comparar el tamaño de una barra de Psfao con una de "
            "Moffat no significa nada.\n\n"
            "`C` va en escala logarítmica porque recorre decenas de órdenes de magnitud entre "
            "bins vecinos (§8): no es una escala fea, es el síntoma."
        ),
        code(
        ...
        md(
            ...
            "La vista global. Un panel por parámetro; **x = λ**, **y = el cubo ordenado por "
            "`DATE-OBS`**, color = el valor. La línea horizontal separa los OB.\n\n"
            "El color es **divergente y centrado en la mediana** del propio parámetro, con los "
            "límites en percentiles robustos: la pregunta de esta sección no es «cuánto vale» "
            "sino «cuánto se aparta de lo típico», y un mapa divergente responde a eso "
            "directamente — el gris del centro *es* «como siempre». Las celdas de ajustes que "
            "no convergieron van en gris claro y **no** son un valor bajo: son un hueco.\n\n"
            "Cómo se lee: **rayas verticales** (una columna entera desviada, en todas las "
            "exposiciones) = estructura cromática real. **Rayas horizontales** (una exposición "
            "entera desviada) = esa observación es distinta —seeing, masa de aire—. **Moteado "
            "sin estructura** = ruido de ajuste, y ahí suavizar en λ no borra nada."
        ),
        code(
        ...
        md(
            ...
            "Un panel barato que interpreta uno de los siete. Los cubos salen todos con el "
            "norte arriba, pero el rotador del instrumento se mueve entre exposiciones: si la "
            "elongación que mide la PSF fuera **instrumental**, `theta` tendría que seguir a "
            "`ADA POSANG`; si es del cielo (o ruido de un parámetro mal determinado), no."
        ),
        code(
        ...
        md(
            ...
            "**El discriminante es la REPRODUCIBILIDAD, no la amplitud.** A cada cubo se le "
            "quita su propia mediana —para matar el desplazamiento entre exposiciones— y se "
            "pregunta si lo que queda, el perfil en λ, **es el mismo en todas**. El ruido no se "
            "repite; la física sí. Se mide con la `r` media entre los perfiles de todos los "
            "pares de cubos.\n\n"
            "> **Por qué no vale comparar amplitudes, que es lo que esta celda hacía antes.** "
            "El primer diseño enfrentaba «cuánto salta en λ» contra «cuánto se dispersa entre "
            "exposiciones» y llamaba ruido a lo primero si era menor. La hipótesis nula estaba "
            "mal: la dispersión entre exposiciones **no es ruido de medida**, es variación real "
            "de la PSF —el seeing y la AO cambian de una exposición a otra—, así que "
            "sobreestima el suelo de ruido y hace demasiado fácil llamar «ruido» a estructura "
            "cromática de verdad. Le pasó a `r0`, que se llevó un «indistinguible del ruido» "
            "teniendo la ley de Fried entera dentro.\n\n"
            "> **Y el «salto en λ» se medía mal.** Era el **MAD de las diferencias** entre bins "
            "contiguos, que es la *dispersión* de los saltos, no su *tamaño*: para una rampa "
            "monótona perfecta todos los saltos son iguales, su MAD es exactamente cero y la "
            "tabla la etiquetaba «sin estructura en λ». Ahora es la **mediana del valor "
            "absoluto** del salto. Las dos columnas de amplitud siguen ahí, pero como contexto, "
            "no como veredicto.\n\n"
            "Dos correcciones más que la tabla aplica y sin las cuales engaña:\n\n"
            "* **Las orientaciones van envueltas a su periodo** (`theta`, `theta_deg`). Una "
            "elipse a 89° y otra a −89° distan 2°, no 178°; sin envolver, `theta` salía con "
            "una dispersión del 200 % y un veredicto de «estructura real» que era el salto de "
            "periodo. Su escala de referencia es medio periodo, no la mediana.\n"
            "* **La columna `pegado`** es la fracción de celdas clavadas exactamente en el "
            "extremo del parámetro. Un parámetro recortado a su límite físico (§8) repite el "
            "mismo número en muchas celdas, su dispersión entre exposiciones se va a cero y el "
            "cociente se dispara sin que haya nada medido — a `C` le sale un cociente de seis "
            "cifras con un 0.00 % de dispersión entre exposiciones. Por encima del 25 % pegado "
            "(o con la dispersión por debajo del 0.1 % de la escala) la fila dice "
            "`DEGENERADO` en vez de dar un veredicto falso.\n\n"
            "Y un **control físico** al final, que es lo que convierte «el perfil se repite» en "
            "«y además es el perfil que toca»: `r0` es el parámetro de Fried y tiene que ir como "
            "**λ^(6/5)**. Si el ajuste lo recupera exposición por exposición, la estructura "
            "cromática de `r0` no es que sea real: es que es *conocida*, y un modelo que la "
            "imponga en vez de re-ajustarla bin a bin no pierde nada."
        ),
        code(
        ...
        md(
            ...
            "la etapa quien lo sube a `True` después (§11).\n\n"
        ...
        md(
            "## 13 · Prototipo: bloquear lo degenerado\n\n"
            "Las §§7-8 dejan un hecho crudo: los siete parámetros **saltan entre bins vecinos "
            "mucho más de lo que permite su propio error formal**. Esta sección prueba la "
            "hipótesis obvia —que el ajuste se está deslizando por un valle degenerado y que "
            "fijando lo que no debe moverse el modelo mejora— y la mide contra un criterio "
            "externo, no estético.\n\n"
            "**El criterio.** El cromatismo del halo: `F(≤32 px) / F(≤25 px)`, medido en el dato "
            "(dos sumas de apertura, sin modelo) contra el que predice el modelo. Es la §12 de "
            "`apcorr_debug`, y es lo que deforma el continuo de C2/C3/C4. Que las curvas de "
            "parámetros salgan más lisas no vale de nada si ese número no se mueve.\n\n"
            "**Las variantes**, todas con los mismos bins que el resto del notebook "
            "(`PASO_BINS`), para que la comparación sea justa:\n\n"
            "| | libres | bloqueados |\n|---|---|---|\n"
            "| **P0** | los 7 | — (es la cadena de hoy) |\n"
            "| **P1** | `r0`, `C`, `A`, `alpha` | elipse y `beta`, al ancla |\n"
            "| **P2** | `C`, `A`, `alpha` | los de P1 + `r0` por la ley de Fried |\n\n"
            "Nada de esto toca `musepipe`: `maoppy.psffit` acepta `fixed=`, y aquí se usa esa "
            "puerta con la copia local."
        ),
        code(
            "if not ES_PSFAO:\n"
            "    print('esta cadena eligió Moffat: no hay parámetros de PSD que bloquear.')\n"
            "    print('La sección entera es de la rama psfao.')\n"
            "else:\n"
            "    _lam = np.array([float(r['lambda_A']) for r in FILAS_PSFAO])\n"
            "    _o = np.argsort(_lam); _lam = _lam[_o]\n"
            "    _v = {n: np.array([float(r[n]) for r in FILAS_PSFAO])[_o] for n in NOMBRES}\n"
            "    _e = {n: np.array([float(r.get(f'{n}_err', np.nan) or np.nan)\n"
            "                       for r in FILAS_PSFAO])[_o] for n in NOMBRES}\n"
            "    # OJO con como se mide el «temblor». La primera diferencia de una rampa\n"
            "    # monotona no es ruido: es la pendiente. La §10.e ya se llevo ese golpe\n"
            "    # (le puso «indistinguible del ruido» a r0, que tiene la ley de Fried\n"
            "    # entera dentro). Aqui van las dos, y la SEGUNDA diferencia -- que una\n"
            "    # rampa perfecta deja en cero -- es la que mide temblor de verdad.\n"
            "    from maoppy.psfmodel import Psfao as _Psfao\n\n"
            "    _lim_lo, _lim_hi = _Psfao((52, 52), system=SYSTEM,\n"
            "                              samp=float(SYSTEM.samp(7000e-10))).bounds\n"
            "    print(f'{\"param\":7s} {\"|dif1|/err\":>10s} {\"|dif2|/err\":>10s}'\n"
            "          f' {\"R2 suave\":>9s}  {\"en el limite\":>12s}')\n"
            "    print(f'{\"\":7s} {\"rampa+ruido\":>10s} {\"solo ruido\":>10s}')\n"
            "    for _k, n in enumerate(NOMBRES):\n"
            "        _vv, _ee = _v[n], _e[n]\n"
            "        with np.errstate(invalid='ignore', divide='ignore'):\n"
            "            _d1 = np.nanmedian(np.abs(np.diff(_vv)) / np.maximum(_ee[1:], 1e-30))\n"
            "            _d2 = np.nanmedian(np.abs(np.diff(_vv, 2)) / np.maximum(_ee[2:], 1e-30))\n"
            "        _r2 = 1 - np.var(_vv - np.polyval(np.polyfit(_lam, _vv, 2), _lam)) / np.var(_vv)\n"
            "        _tol = 1e-9 + 1e-6 * max(abs(np.median(_vv)), 1e-30)\n"
            "        _peg = 0\n"
            "        for _b in (_lim_lo[_k], _lim_hi[_k]):\n"
            "            if np.isfinite(_b):\n"
            "                _peg += int(np.sum(np.abs(_vv - _b) <= max(_tol, 1e-7)))\n"
            "        print(f'   {n:6s} {_d1:8.1f} {_d2:10.1f} {_r2:10.2f}'\n"
            "              f'   {_peg:3d}/{_vv.size} bins')\n"
            "    print()\n"
            "    print('   |dif2| grande CON R2 alto = una curva suave con temblor encima;')\n"
            "    print('   |dif2| grande con R2 bajo = el parametro no describe nada;')\n"
            "    print('   «en el limite» = el ajuste lo empujo contra su cota y ahi no se mide.')\n"
            "    print('   Y los errores formales son 1/sqrt(diag(JtJ)) con pesos 1/STAT, que')\n"
            "    print('   `docs/noise_model.md` tiene medido que subestima: son COTA INFERIOR,')\n"
            "    print('   asi que un x20 no significa «veinte sigmas de verdad».')\n"
            "    print()\n"
            "    # theta: la ambiguedad (ratio, theta) <-> (1/ratio, theta+90) es la MISMA\n"
            "    # elipse. Si los saltos grandes caen justo donde `ratio` cruza 1, no son\n"
            "    # un mal ajuste: son la etiqueta de los ejes intercambiandose.\n"
            "    _th = np.degrees(_v['theta']); _rt = _v['ratio']\n"
            "    _dth = np.abs(np.diff(_th)); _dth = np.minimum(_dth, 180 - _dth)\n"
            "    _cruza = (_rt[:-1] - 1) * (_rt[1:] - 1) < 0\n"
            "    _grandes = _dth > 20\n"
            "    print(f'saltos de theta > 20 deg: {int(_grandes.sum())};'\n"
            "          f' de ellos, con `ratio` cruzando 1: {int((_grandes & _cruza).sum())}')\n"
            "    for i in np.argsort(_dth)[::-1][:5]:\n"
            "        print(f'   {_lam[i]:6.0f}->{_lam[i+1]:6.0f} A: dtheta={_dth[i]:5.1f} deg'\n"
            "              f' | ratio {_rt[i]:.3f}->{_rt[i+1]:.3f}'\n"
            "              f\"{'   <- cruza 1' if _cruza[i] else ''}\")\n"
            "    # La misma elipse en coordenadas invariantes, que no tienen la ambiguedad.\n"
            "    _q = np.where(_rt >= 1, _rt, 1 / _rt)\n"
            "    _phi = np.where(_rt >= 1, _v['theta'], _v['theta'] + np.pi / 2)\n"
            "    _eps = (_q - 1) / (_q + 1)\n"
            "    E1, E2 = _eps * np.cos(2 * _phi), _eps * np.sin(2 * _phi)\n"
            "    print()\n"
            "    print(f'la misma elipse en (e1,e2): elongación {100 * np.median(_q - 1):.1f}%,'\n"
            "          f' salto mediano {np.median(np.abs(np.diff(E1))):.4f} /'\n"
            "          f' {np.median(np.abs(np.diff(E2))):.4f}')\n"
            "    print('   -> si estos saltos son minúsculos, la elipse NO se mueve:'\n"
            "          ' lo que salta es el nombre de los ejes.')\n"
            "    print()\n"
            "    print(f\"alpha-beta: r = {np.corrcoef(_v['alpha'], _v['beta'])[0, 1]:+.3f}\"\n"
            "          '  <- degeneración de verdad: dos parámetros para una libertad')\n"
            "    # Para la §13.e: el coste y el anillo de la cadena, en el mismo orden.\n"
            "    _cost_csv = np.array([float(r['optimizer_cost']) for r in FILAS_PSFAO])[_o]\n"
            "    _ring_csv = np.array([float(r['ring_residual_pct']) for r in FILAS_PSFAO])[_o]"
        ),
        md(
            "### 13.a · El ancla, y por qué hay dos\n\n"
            "**La del cubo mediano.** Ajustar la mediana en λ del cubo debería dar un modelo más "
            "estable que cualquier bin. Con una trampa: la mediana pondera los canales por el "
            "**espectro de la estrella**, así que la λ efectiva no es la que uno cree — hay que "
            "normalizar cada canal por su flujo antes. Y con una limitación de fondo: la mediana "
            "de PSFs de anchuras distintas **no es una PSF**, es una mezcla, siempre más ancha y "
            "con más falda que cualquiera de sus componentes.\n\n"
            "**La de la mediana de los bins.** La alternativa es no ajustar nada nuevo: tomar la "
            "mediana robusta de los ajustes por bin que ya existen, con la elipse medianada en "
            "(e₁, e₂) para que el intercambio de ejes no envenene la mediana.\n\n"
            "Las dos se calculan aquí y se comparan por su residuo de anillo. La que ajuste peor "
            "no sirve de ancla, por buena que suene."
        ),
        code(
            "import time\n\n"
            "if ES_PSFAO:\n"
            "    _t0 = time.time()\n"
            "    _flujo = np.nansum(CUBE_SUB, axis=(1, 2))\n"
            "    _flujo = np.where(np.isfinite(_flujo) & (_flujo > 0), _flujo, np.nan)\n"
            "    _img_anc = np.nanmedian(CUBE_SUB / _flujo[:, None, None], axis=0)\n"
            "    _var_anc = np.nanmedian(STAT_SUB / np.clip(_flujo, 1e-30, None)[:, None, None] ** 2,\n"
            "                            axis=0)\n"
            "    LAM_REF = float(np.nanmedian(WAVE_ALL[CANALES_SUB]))\n"
            "    with contextlib.redirect_stdout(_io.StringIO()), _w.catch_warnings():\n"
            "        _w.simplefilter('ignore')\n"
            "        _par, _amp, _bck, _dxdy, _ring_anc, _rec, _opt, _err = fit_bin(\n"
            "            _img_anc, _var_anc, float(SYSTEM.samp(LAM_REF * 1e-10)), SYSTEM,\n"
            "            COMP_YX, MASK_RADIUS_PX, FIT_RADIUS_PX, list(DEFAULT_X0),\n"
            "            field_yx=FIELD_YX)\n"
            "    ANCLA_CUBO = dict(zip(NOMBRES, map(float, _par)))\n"
            "    if ANCLA_CUBO['ratio'] < 1:      # convención ratio >= 1\n"
            "        ANCLA_CUBO['ratio'] = 1 / ANCLA_CUBO['ratio']\n"
            "        ANCLA_CUBO['theta'] = ANCLA_CUBO['theta'] + np.pi / 2\n"
            "    ANCLA_CUBO['theta'] = float(np.mod(ANCLA_CUBO['theta'], np.pi))\n\n"
            "    _elip = np.hypot(np.median(E1), np.median(E2))\n"
            "    ANCLA_BINS = {n: float(np.median(_v[n])) for n in NOMBRES}\n"
            "    ANCLA_BINS['ratio'] = float((1 + _elip) / (1 - _elip))\n"
            "    ANCLA_BINS['theta'] = float(np.mod(0.5 * np.arctan2(np.median(E2),\n"
            "                                                        np.median(E1)), np.pi))\n\n"
            "    print(f'ancla del cubo mediano (λ_ref = {LAM_REF:.0f} Å,'\n"
            "          f' {time.time() - _t0:.0f} s): residuo de anillo {_ring_anc:.1f} %')\n"
            "    print(f'residuo de anillo mediano de los bins sueltos:'\n"
            "          f\" {np.nanmedian([float(r['ring_residual_pct']) for r in FILAS_PSFAO]):.1f} %\")\n"
            "    print()\n"
            "    print(f'{\"\":8s} {\"cubo mediano\":>14s} {\"mediana de bins\":>16s}')\n"
            "    for n in NOMBRES:\n"
            "        print(f'   {n:6s} {ANCLA_CUBO[n]:14.5g} {ANCLA_BINS[n]:16.5g}')\n"
            "    if not np.isfinite(_ring_anc) or _ring_anc > 2 * np.nanmedian(\n"
            "            [float(r['ring_residual_pct']) for r in FILAS_PSFAO]):\n"
            "        print()\n"
            "        print('   -> el ancla del cubo mediano ajusta MUCHO peor que un bin suelto:')\n"
            "        print('      es la mezcla de anchuras. Se usan las dos y se comparan.')"
        ),
        md(
            "### 13.b · Las tres variantes, y el número que decide\n\n"
            "`maoppy.psffit` acepta `fixed=`, un booleano por parámetro. El resto del ajuste "
            "—máscara del compañero, pesos `1/STAT`, radio— es **idéntico** al de `fit_bin`, "
            "que viaja copiado arriba: lo único que cambia es qué se deja mover."
        ),
        code(
            "import time\n\n\n"
            "def _fit_fijo(img, var, samp, x0, fijos, pesos='ctrl'):\n"
            "    \"\"\"`fit_bin` con parámetros bloqueados y/o otro esquema de pesos.\n\n"
            "    `pesos='ctrl'` son exactamente los de la cadena (1/STAT). Los otros\n"
            "    dos son la §13.f: `'rel'` pesa por error RELATIVO —cada anillo cuenta\n"
            "    lo mismo aunque sea mil veces más débil— y `'halo'` enmascara el\n"
            "    núcleo y ajusta SOLO el halo.\n"
            "    \"\"\"\n"
            "    from maoppy.psffit import psffit\n"
            "    from maoppy.psfmodel import Psfao\n\n"
            "    fixed = np.array([n in fijos for n in NOMBRES], dtype=bool)\n"
            "    ny, nx = img.shape\n"
            "    yy, xx = np.mgrid[0:ny, 0:nx]\n"
            "    _r = np.hypot(yy - ny // 2, xx - nx // 2)\n"
            "    _c = np.hypot(yy - COMP_YX[0], xx - COMP_YX[1])\n"
            "    _m = np.isfinite(img) & (_c > MASK_RADIUS_PX) & (_r < FIT_RADIUS_PX)\n"
            "    if FIELD_YX is not None:\n"
            "        _m &= np.hypot(yy - FIELD_YX[0], xx - FIELD_YX[1]) > MASK_RADIUS_PX\n"
            "    _wt = 1 / np.clip(var, 1e-6, None)   # `_w` ya es el alias de warnings\n"
            "    if pesos == 'rel':\n"
            "        _piso = np.nanpercentile(np.abs(img), 50)\n"
            "        _wt = _wt / np.clip(np.abs(img), max(_piso, 1e-6), None) ** 2\n"
            "    elif pesos == 'halo':\n"
            "        _wt = np.where(_r >= R_NUCLEO_PX, _wt, 0.0)\n"
            "    _mod = Psfao((ny, nx), system=SYSTEM, samp=float(samp))\n"
            "    with contextlib.redirect_stdout(_io.StringIO()), _w.catch_warnings():\n"
            "        _w.simplefilter('ignore')\n"
            "        res = psffit(np.where(np.isfinite(img), img, 0.0), _mod, x0,\n"
            "                     weights=np.where(_m, _wt, 0.0),\n"
            "                     flux_bck=(True, True),\n"
            "                     fixed=fixed if fixed.any() else None, max_nfev=400)\n"
            "    _amp, _bck = res.flux_bck\n"
            "    _recon = _amp * _mod(res.x, dx=res.dxdy[0], dy=res.dxdy[1]) + _bck\n"
            "    # El residuo del anillo que fija la apcorr, con la reconstrucción\n"
            "    # COMPLETA (amp·PSF + bck): comparar el dato contra la PSF\n"
            "    # normalizada daría un número sin sentido, porque al dato no se le\n"
            "    # ha quitado el fondo que el ajuste sí modela.\n"
            "    _an = ((_r > NORM_RADIUS_PX) & (_r <= R_GRANDE_PX)\n"
            "           & (_c > MASK_RADIUS_PX) & np.isfinite(img))\n"
            "    _dato_an = float(np.nansum(img[_an]))\n"
            "    _res_an = (1 - float(np.nansum(_recon[_an])) / _dato_an\n"
            "               if _dato_an else np.nan)\n"
            "    return (list(map(float, res.x)),\n"
            "            _ring_residual(img, _recon, COMP_YX, MASK_RADIUS_PX),\n"
            "            float(res.cost), float(_res_an))\n\n\n"
            "def _pasada(fijos, x0_de, etiqueta, pesos='ctrl'):\n"
            "    \"\"\"Un ajuste por cada bin de `IDX_BINS`, con `fijos` bloqueados.\"\"\"\n"
            "    filas = []\n"
            "    _t = time.time()\n"
            "    for i in IDX_BINS:\n"
            "        a, b, mid, sel = BINS_PSFAO[i]\n"
            "        _sub = np.zeros(CANALES_SUB.size, dtype=bool)\n"
            "        _sub[[_pos[int(c)] for c in np.where(sel)[0]]] = True\n"
            "        par, ring, cost, res_an = _fit_fijo(\n"
            "            np.nanmedian(CUBE_SUB[_sub], axis=0),\n"
            "            np.nanmedian(STAT_SUB[_sub], axis=0),\n"
            "            float(SYSTEM.samp(mid * 1e-10)),\n"
            "            list(x0_de(mid)), fijos, pesos=pesos)\n"
            "        filas.append(dict(zip(NOMBRES, par), lambda_A=float(mid),\n"
            "                          ring=float(ring), cost=float(cost),\n"
            "                          resid_anillo=float(res_an)))\n"
            "    print(f'   {etiqueta}: {len(filas)} bins en {time.time() - _t:.0f} s')\n"
            "    return filas\n\n\n"
            "def _croma(filas):\n"
            "    \"\"\"El cromatismo F(<=32)/F(<=25) que predice ese juego de parámetros.\"\"\"\n"
            "    from musepipe.extraction.aperture import aperture_correction_from_psf\n\n"
            "    doc = {'form': 'psfao', 'system': SYSTEM_NAME, 'param_names': list(NOMBRES),\n"
            "           'norm_radius_px': NORM_RADIUS_PX, 'psfao_wave_bin_A': float(BIN_A),\n"
            "           'param_table': {'lambda_A': [f['lambda_A'] for f in filas],\n"
            "                           **{n: [f[n] for f in filas] for n in NOMBRES}}}\n"
            "    _w_sub = WAVE_ALL[CANALES_SUB]\n"
            "    _cg, _, _ = aperture_correction_from_psf(\n"
            "        _w_sub, {'kind': 'circle', 'radius_px': R_GRANDE_PX, 'name': 'g'}, doc,\n"
            "        center_yx=STAR_YX, correction_mode='auto', growth_curve=None)\n"
            "    _cn, _, _ = aperture_correction_from_psf(\n"
            "        _w_sub, {'kind': 'circle', 'radius_px': NORM_RADIUS_PX, 'name': 'n'}, doc,\n"
            "        center_yx=STAR_YX, correction_mode='auto', growth_curve=None)\n"
            "    return _rango(_cn / _cg)\n\n\n"
            "def _rango(v):\n"
            "    _f = np.isfinite(v)\n"
            "    return float(np.nanpercentile(v[_f], 98)) / float(np.nanpercentile(v[_f], 2))\n\n\n"
            "def _panel_parametros(series, titulo):\n"
            "    \"\"\"Los siete parámetros contra λ, una curva por variante.\n\n"
            "    Cada panel elige escala lineal o `symlog` segun el recorrido de sus\n"
            "    valores: `C` cruza treinta ordenes de magnitud y en lineal no se ve\n"
            "    nada, y forzar log a todos aplasta a los que se mueven un 5 %.\n"
            "    \"\"\"\n"
            "    _f = (len(NOMBRES) + 2) // 2\n"
            "    fig, axes = plt.subplots(_f, 2, figsize=(11.5, 1.9 * _f + 1.4), sharex=True,\n"
            "                             squeeze=False)\n"
            "    for ax in axes.ravel()[len(NOMBRES):]:\n"
            "        ax.axis('off')\n"
            "    for ax, n in zip(axes.ravel(), NOMBRES):\n"
            "        for _et, _fl, _col, _lw, _ls in series:\n"
            "            _l = np.array([f['lambda_A'] for f in _fl])\n"
            "            _o3 = np.argsort(_l)\n"
            "            ax.plot(_l[_o3], np.array([f[n] for f in _fl])[_o3], _ls, lw=_lw,\n"
            "                    color=_col, marker='o', ms=2.5, label=_et)\n"
            "        _vs = np.concatenate([np.abs(np.array([f[n] for f in _fl]))\n"
            "                              for _, _fl, _, _, _ in series])\n"
            "        _vs = _vs[np.isfinite(_vs) & (_vs > 0)]\n"
            "        if _vs.size and _vs.max() / _vs.min() > 1e3:\n"
            "            ax.set_yscale('symlog', linthresh=max(_vs.min(), 1e-8))\n"
            "        ax.set_title(n, fontsize=9, loc='left')\n"
            "        ax.tick_params(labelsize=8)\n"
            "    axes.ravel()[0].legend(fontsize=7.5, ncol=2)\n"
            "    for ax in axes.ravel()[max(0, len(NOMBRES) - 2):len(NOMBRES)]:\n"
            "        ax.set_xlabel('λ [Å]')\n"
            "    fig.suptitle(titulo, fontsize=10)\n"
            "    fig.tight_layout()\n"
            "    plt.show()\n\n\n"
            "R_GRANDE_PX = 32.0   # apertura grande: recoge casi todo el halo medible\n"
            "R_NUCLEO_PX = 12.0   # lo que la variante «solo halo» de la §13.f enmascara\n"
            "if ES_PSFAO:\n"
            "    # La vara: el cromatismo del DATO, dos sumas de apertura y su cociente.\n"
            "    with np.errstate(invalid='ignore', divide='ignore'):\n"
            "        CROMA_DATO = _rango(extract_aperture_spectrum(CUBE_SUB, STAR_YX, R_GRANDE_PX)\n"
            "                            / extract_aperture_spectrum(CUBE_SUB, STAR_YX,\n"
            "                                                       NORM_RADIUS_PX))\n"
            "    _lam_sub = [float(BINS_PSFAO[i][2]) for i in IDX_BINS]\n"
            "    P0 = [dict({n: float(r[n]) for n in NOMBRES}, lambda_A=float(r['lambda_A']),\n"
            "               ring=float(r['ring_residual_pct'])) for r in FILAS_PSFAO\n"
            "          if any(abs(float(r['lambda_A']) - w) < 1e-6 for w in _lam_sub)]\n"
            "    _x0b = [ANCLA_BINS[n] for n in NOMBRES]\n"
            "    P1 = _pasada({'ratio', 'theta', 'beta'}, lambda mid: _x0b, 'P1')\n"
            "    _lam1 = np.array([f['lambda_A'] for f in P1])\n"
            "    _r01 = np.array([f['r0'] for f in P1])\n"
            "    _ley = (_lam1 / LAM_REF) ** 1.2      # Fried: r0 ~ lambda^(6/5)\n"
            "    R0_REF = float(np.sum(_r01 * _ley) / np.sum(_ley ** 2))\n"
            "    P2 = _pasada({'ratio', 'theta', 'beta', 'r0'},\n"
            "                 lambda mid: [R0_REF * (mid / LAM_REF) ** 1.2 if n == 'r0'\n"
            "                              else ANCLA_BINS[n] for n in NOMBRES], 'P2')\n"
            "    print()\n"
            "    print(f'cromatismo medido en el DATO: {CROMA_DATO:.4f}')\n"
            "    print()\n"
            "    print(f'{\"variante\":34s} {\"cromatismo\":>11s} {\"% del dato\":>11s}'\n"
            "          f' {\"anillo\":>8s}')\n"
            "    for _et, _fl in (('P0 · los 7 libres (la cadena)', P0),\n"
            "                     ('P1 · elipse y beta bloqueados', P1),\n"
            "                     ('P2 · P1 + r0 por ley de Fried', P2)):\n"
            "        _c = _croma(_fl)\n"
            "        print(f'{_et:34s} {_c:11.4f}'\n"
            "              f' {100 * (_c - 1) / (CROMA_DATO - 1):10.0f}%'\n"
            "              f\" {np.nanmedian([f['ring'] for f in _fl]):7.2f}%\")\n"
            "    _panel_parametros(\n"
            "        [('P0 · la cadena', P0, '0.55', 0.9, '-'),\n"
            "         ('P1 · elipse y beta fijos', P1, 'tab:blue', 1.1, '-'),\n"
            "         ('P2 · + r0 por Fried', P2, 'tab:red', 1.2, '--')],\n"
            "        'los siete parámetros: la cadena contra las dos variantes'\n"
            "        '  (una línea plana = parámetro bloqueado)')"
        ),
        md(
            "### 13.c · El experimento que lo decide\n\n"
            "Si bloquear no mueve el cromatismo, la pregunta siguiente es si **algo** dentro de "
            "esta parametrización puede darlo. `r0` es el único parámetro con una ley física "
            "clara —Fried, r₀ ∝ λ^(6/5)— así que se barre el exponente: se congela todo a su "
            "mediana, se impone `r0 = r0_ref · (λ/λ_ref)^p` y se mira el cromatismo que sale. "
            "**Sin reajustar nada**: es la evaluación del modelo, no un ajuste, así que es "
            "barato.\n\n"
            "`p = 0` es el suelo del modelo: todos los parámetros constantes. Lo que quede ahí "
            "es cromatismo **de la difracción**, porque la PSF se construye al muestreo de cada "
            "λ. El resultado interesante es cuánto hay que forzar `p` para llegar al dato — y si "
            "ese `p` sigue siendo física."
        ),
        code(
            "if ES_PSFAO:\n"
            "    _base = {n: float(np.median(_v[n])) for n in NOMBRES}\n"
            "    _lam_p = np.array([f['lambda_A'] for f in P0])\n"
            "    print(f'{\"p\":>6s} {\"cromatismo\":>11s}   (dato {CROMA_DATO:.4f})')\n"
            "    _sal = []\n"
            "    for _p in (0.0, 1.2, 2.0, 3.0, 4.0):\n"
            "        _fl = [dict(_base, lambda_A=float(w), ring=np.nan,\n"
            "                    r0=float(_base['r0'] * (w / LAM_REF) ** _p)) for w in _lam_p]\n"
            "        _c = _croma(_fl); _sal.append((_p, _c))\n"
            "        print(f'{_p:6.2f} {_c:11.4f}'\n"
            "              f\"{'   <- Fried' if abs(_p - 1.2) < 1e-9 else ''}\"\n"
            "              f\"{'   <- todo constante: solo difracción' if _p == 0 else ''}\")\n"
            "    _ps = np.array([p for p, _ in _sal]); _cs = np.array([c for _, c in _sal])\n"
            "    _sube = _cs > _cs[np.argmin(_cs)]\n"
            "    if CROMA_DATO < _cs.max():\n"
            "        _i = np.argmax(_cs >= CROMA_DATO)\n"
            "        _p_nec = np.interp(CROMA_DATO, _cs[_i - 1:_i + 1], _ps[_i - 1:_i + 1])\n"
            "        print()\n"
            "        print(f'para igualar el dato haría falta p ≈ {_p_nec:.2f},'\n"
            "              f' {_p_nec / 1.2:.1f}× el exponente de Fried')"
        ),
        md(
            "### 13.f · Pedirle el halo al ajuste\n\n"
            "Las variantes de arriba cambian **qué parámetros se mueven**. Ésta cambia otra "
            "cosa: **qué parte de la imagen decide el ajuste**.\n\n"
            "`fit_bin` pesa con `1/STAT` sobre todo el disco de `FIT_RADIUS_PX`, y eso lo domina "
            "el **núcleo** por varios órdenes de magnitud: el halo aporta una fracción "
            "despreciable del χ². Dicho de otro modo, al modelo **nunca se le ha pedido el "
            "halo** — y luego se le reprocha no reproducirlo. Dos maneras de pedírselo, sin "
            "tocar ni un parámetro ni una cota:\n\n"
            "| | pesos | qué significa |\n|---|---|---|\n"
            "| **W_REL** | `1/STAT` ÷ `max(imagen, piso)²` | error **relativo**: cada anillo "
            "cuenta lo mismo aunque sea mil veces más débil |\n"
            "| **W_HALO** | `1/STAT`, con `r < R_NUCLEO_PX` a cero | **sólo el halo**: el núcleo "
            "no entra en el ajuste |\n\n"
            "El control es el mismo ajuste con los pesos de la cadena y el mismo arranque. Y se "
            "mide con el mismo criterio externo de la §13.b —el cromatismo de la **PSF "
            "normalizada**, que es lo único que C2–C6 evalúan— más el residuo del anillo "
            "25–32 px, que es donde la §5 de `residuos_debug` localizó el problema.\n\n"
            "> `W_HALO` es lento: al quitarle el núcleo, el optimizador necesita muchas más "
            "iteraciones por bin."
        ),
        code(
            "if ES_PSFAO:\n"
            "    _x0f = [ANCLA_BINS[n] for n in NOMBRES]\n"
            "    PESADOS = {}\n"
            "    for _et, _pw in (('CTRL  · 1/STAT (la cadena)', 'ctrl'),\n"
            "                     ('W_REL · error relativo', 'rel'),\n"
            "                     ('W_HALO· sólo el halo', 'halo')):\n"
            "        PESADOS[_et] = _pasada(set(), lambda mid: _x0f, _et.split('·')[0].strip(),\n"
            "                               pesos=_pw)\n"

            "    print()\n"
            "    print(f'{\"variante\":26s} {\"croma PSF\":>10s} {\"% del dato\":>11s}'\n"
            "          f' {\"anillo\":>8s} {\"resid 25-32\":>12s}')\n"
            "    for _et, _fl in PESADOS.items():\n"
            "        _c = _croma(_fl)\n"
            "        print(f'{_et:26s} {_c:10.4f}'\n"
            "              f' {100 * (_c - 1) / (CROMA_DATO - 1):10.0f}%'\n"
            "              f\" {np.nanmedian([f['ring'] for f in _fl]):7.2f}%\"\n"
            "              f\" {np.nanmedian([f['resid_anillo'] for f in _fl]):11.2%}\")\n"
            "    if PASO_BINS > 1:\n"
            "        print()\n"
            "        print(f'OJO: con PASO_BINS={PASO_BINS} la tabla de parámetros que se'\n"
            "              ' interpola tiene huecos de'\n"
            "              f' {PASO_BINS * BIN_A:.0f} Å, y eso SUAVIZA el cromatismo.')\n"
            "        print('     El efecto es mayor cuanto más se muevan los parámetros, así que'\n"
            "              ' castiga')\n"
            "        print('     sobre todo a W_HALO. Para citar un número, PASO_BINS = 1.')\n"
            "    _panel_parametros(\n"
            "        [(_et, _fl, _c, 1.1, _ls) for (_et, _fl), _c, _ls in zip(\n"
            "            PESADOS.items(), ('0.55', 'tab:blue', 'tab:red'), ('-', '-', '--'))],\n"
            "        'los siete parámetros según qué parte de la imagen decide el ajuste')"
        ),
        md(
            "**Cómo leer esta tabla, y qué NO dice.**\n\n"
            "Si al cambiar sólo los pesos el cromatismo sube, **el modelo sí podía** y lo que "
            "fallaba era el ajuste: la conclusión de la §13.c —«haría falta `p ≈ 2.4`, el doble "
            "de Fried»— se midió congelando los demás parámetros en las medianas del ajuste "
            "actual, o sea **dentro del valle equivocado**, y no vale como prueba de que la "
            "familia Psfao no llegue.\n\n"
            "Dos cautelas antes de darlo por bueno:\n\n"
            "1. **Mirar si algún parámetro acaba pegado a su cota.** Un cromatismo que sólo se "
            "alcanza con `beta` en su tope no es un ajuste, es un borde — y la celda de "
            "diagnóstico de la §13 tiene la columna «en el límite» para verlo.\n"
            "2. **El nivel y la forma son cosas distintas.** Reproducir el *recorrido* del "
            "cromatismo no implica que el residuo del anillo se vaya a cero; puede quedar un "
            "desplazamiento constante, y eso es otro problema.\n\n"
            "> Y nada de esto cambia la cadena: cambiar los pesos de C1 obliga a re-correr "
            "C1 → C2–C6 → D2 → E → F → G, y el `psf_systematic_fraction` que se publica es una "
            "decisión aprobada. Aquí sólo está el número."
        ),
        md(
            "### 13.d · Qué contesta esto, y qué no\n\n"
            "Lo que la sección **mide** (los números salen arriba, en este run):\n\n"
            "0. **Cuidado con llamar «temblor» a la primera diferencia.** Una rampa monótona "
            "tiene saltos grandes entre bins y no tiene ruido ninguno: es lo que le pasa a "
            "`r0`, que sube liso de 0.09 a 0.21 y aun así da ×28 su error formal. La segunda "
            "diferencia y el R² de la celda de diagnóstico separan las dos cosas, y los "
            "paneles de parámetros lo enseñan de un vistazo.\n"
            "1. **Los saltos de `theta` no son un mal ajuste.** Si todos los grandes coinciden "
            "con `ratio` cruzando 1, son la ambigüedad de la parametrización, y la elipse en "
            "(e₁, e₂) está quieta. Se arregla reparametrizando, sin ningún prior.\n"
            "2. **`alpha`-`beta` sí es degeneración**, y bloquear uno está justificado — pero "
            "mirar si el temblor **se muda** al que queda libre en vez de desaparecer.\n"
            "3. **Que P1 y P2 no muevan el cromatismo NO prueba que el modelo no pueda.** "
            "Prueba que no lo arregla *re-parametrizando*, que es otra cosa. Y si P2 (la física "
            "correcta, Fried) lo empeora, quiere decir que los ajustes libres estaban "
            "**fingiendo** parte del cromatismo con una `r0` más inclinada de lo que permite la "
            "atmósfera.\n"
            "4. **El barrido de `p` se hizo dentro del valle equivocado.** Congela los otros "
            "seis parámetros en las medianas del ajuste ACTUAL y mueve uno: dice cuánto da ese "
            "eje desde ahí, no cuánto da la familia Psfao. La §13.f lo desmiente cambiando sólo "
            "los pesos.\n\n"
            "Lo que **no** contesta: de dónde sale el cromatismo que falta. Refracción "
            "diferencial, luz dispersada en el instrumento, vibraciones — nada de eso está "
            "medido aquí, y no se debe escribir como si lo estuviera.\n\n"
            "> Nada de esto entra en la cadena. Es un banco de pruebas: cambia el ajuste dentro "
            "del notebook, mide, y deja el veredicto por escrito. Tocar C1 de verdad obliga a "
            "re-correr C1 → C2-C6 → D2 → E → F → G."
        ),
        md(
            "### 13.e · El suelo plano del PSD (`C`)\n\n"
            "`C` es el suelo constante de la densidad espectral de potencia: una PSD plana "
            "reparte luz por todo el campo, así que `C` gobierna el **pedestal ancho** de la "
            "PSF. Mirando la tabla por bin da la impresión de crecer hacia el rojo — la celda "
            "comprueba qué hace de verdad, y si el ajuste lo **mide** o simplemente lo deja "
            "donde estaba.\n\n"
            "Es el candidato natural a bloquear: si el modelo apenas lo nota, es un grado de "
            "libertad regalado. La prueba son dos variantes contra un control con **el mismo "
            "arranque** (la línea base del CSV usa arranque en caliente, así que no vale de "
            "control), y el precio se mide en **coste del ajuste**, partido azul/rojo, no sólo "
            "en la mediana: si `C` sólo trabaja en una mitad del rango, la mediana global lo "
            "esconde."
        ),
        code(
            "if ES_PSFAO:\n"
            ...
            "    with np.errstate(invalid='ignore', divide='ignore'):\n"
            "        _sig = _Cv / _Ce\n"
            "    _medido = np.isfinite(_sig) & (_sig > 3)\n"
            "    print('C, bin a bin (C/err < 3 = el ajuste NO lo mide):')\n"
            "    for i in range(0, len(_lam), max(1, len(_lam) // 12)):\n"
            "        print(f'   {_lam[i]:6.0f} Å   C={_Cv[i]:10.4g}   C/err={_sig[i]:8.1f}'\n"
            "              f\"{'' if _medido[i] else '   <- no medido'}\")\n"
            "    if _medido.any():\n"
            "        print()\n"
            "        print(f'medido en {int(_medido.sum())} de {_medido.size} bins'\n"
            "              f' ({_lam[_medido].min():.0f}-{_lam[_medido].max():.0f} Å);'\n"
            "              f' pico en {_lam[np.argmax(_Cv)]:.0f} Å')\n"
            "        print(f'   -> donde no se mide, C se queda en su límite inferior (0):'\n"
            "              ' no es «pequeño», es que el dato no lo pide.')\n"
            "    print()\n"
            "    print(f'C vs coste del ajuste: r = {np.corrcoef(_Cv, _cost_csv)[0, 1]:+.2f}'\n"
            "          f' | C vs residuo de anillo: r = {np.corrcoef(_Cv, _ring_csv)[0, 1]:+.2f}')\n"
            "    print('   (si C sube donde el ajuste va peor y el anillo mejor, se está'\n"
            "          ' comportando como comodín)')"
        ),
        code(
            "if ES_PSFAO:\n"
            "    C_MEDIANA = float(np.median(_v['C']))\n"
            "    CTRL = _pasada(set(), lambda mid: _x0b, 'CTRL  (7 libres)')\n"
            "    C_MED = _pasada({'C'}, lambda mid: [C_MEDIANA if n == 'C' else ANCLA_BINS[n]\n"
            "                                        for n in NOMBRES], 'C_MED (C = mediana)')\n"
            "    C_CERO = _pasada({'C'}, lambda mid: [0.0 if n == 'C' else ANCLA_BINS[n]\n"
            "                                         for n in NOMBRES], 'C_CERO(C = 0)')\n"
            "    _CORTE_A = 6500.0   # donde C se enciende en este objeto\n"
            "    print()\n"
            "    print(f'{\"variante\":22s} {\"cromatismo\":>11s} {\"% dato\":>7s}'\n"
            "          f' {\"anillo\":>8s} {\"coste\":>11s} {\"azul\":>11s} {\"rojo\":>11s}')\n"
            "    for _et, _fl in (('CTRL · 7 libres', CTRL),\n"
            "                     (f'C = mediana', C_MED), ('C = 0', C_CERO)):\n"
            "        _l = np.array([f['lambda_A'] for f in _fl])\n"
            "        _k = np.array([f['cost'] for f in _fl])\n"
            "        _c = _croma(_fl)\n"
            "        print(f'{_et:22s} {_c:11.4f}'\n"
            "              f' {100 * (_c - 1) / (CROMA_DATO - 1):6.0f}%'\n"
            "              f\" {np.nanmedian([f['ring'] for f in _fl]):7.2f}%\"\n"
            "              f' {np.nanmedian(_k):11.4g} {np.nanmedian(_k[_l < _CORTE_A]):11.4g}'\n"
            "              f' {np.nanmedian(_k[_l >= _CORTE_A]):11.4g}')\n"
            "    _ctrl = {f['lambda_A']: f['cost'] for f in CTRL}\n"
            "    print()\n"
            "    print('coste relativo al control, bin a bin (1.000 = no cuesta nada):')\n"
            "    for _et, _fl in (('C = mediana', C_MED), ('C = 0', C_CERO)):\n"
            "        _l = np.array([f['lambda_A'] for f in _fl])\n"
            "        _rt = np.array([f['cost'] / _ctrl[f['lambda_A']] for f in _fl])\n"
            "        print(f'   {_et:12s} todo {np.median(_rt):6.3f}'\n"
            "              f'   azul {np.median(_rt[_l < _CORTE_A]):6.3f}'\n"
            "              f'   rojo {np.median(_rt[_l >= _CORTE_A]):6.3f}'\n"
            "              f'   peor bin {_rt.max():6.3f} a {_l[np.argmax(_rt)]:.0f} Å')\n"
            "    print()\n"
            "    print('y el temblor de los que quedan libres (salto / error formal):')\n"
            "    for _et, _fl in (('CTRL', CTRL), ('C = mediana', C_MED), ('C = 0', C_CERO)):\n"
            "        _l = np.array([f['lambda_A'] for f in _fl]); _o2 = np.argsort(_l)\n"
            "        _tr = []\n"
            "        for n in NOMBRES:\n"
            "            _vv = np.array([f[n] for f in _fl])[_o2]\n"
            "            if np.allclose(_vv, _vv[0]):\n"
            "                continue          # bloqueado: no tiene temblor que medir\n"
            "            _tr.append(f'{n}={np.median(np.abs(np.diff(_vv))) / max(np.median(np.abs(_vv)), 1e-30):.3f}')\n"
            "        print(f'   {_et:12s} ' + '  '.join(_tr))\n"
            "    print('   (aquí en unidades de la propia mediana del parámetro: los errores'\n"
            "          ' formales cambian de tamaño al bloquear, y no serían comparables)')\n"
            "    _panel_parametros(\n"
            "        [('P0 · la cadena', P0, '0.55', 0.9, '-'),\n"
            "         ('CTRL · 7 libres', CTRL, 'tab:blue', 1.0, '-'),\n"
            "         ('C = mediana', C_MED, 'tab:orange', 1.1, '--'),\n"
            "         ('C = 0', C_CERO, 'tab:red', 1.3, '-')],\n"
            "        'los siete parámetros con C bloqueado: qué se calma y qué se muda')"
        ),
        md(
            "#### 13.e.2 · `C` contra la frontera del área corregida\n\n"
            "La §7.b de `residuos_debug` localiza el fallo del modelo en el **borde del área "
            "corregida por la AO** (`(Nact/2)·λ/D` ≈ 17 λ/D): dentro le falta luz, fuera le "
            "sobra. Como `C` es justo el suelo de la PSD **dentro** de esa área, es el "
            "candidato inmediato a moverlo.\n\n"
            "La celda barre `C` con los otros seis parámetros congelados y re-ajusta sólo "
            "amplitud y fondo —lineal, con los pesos de la cadena, que es lo que hace `psffit` "
            "con `flux_bck`—, de modo que la comparación es a igualdad de flujo. Mide el radio "
            "del cruce por cero y **lo que cuesta**: `F(≤R_NORM)/F(box3)` contra el dato, que "
            "es la corrección de apertura.\n\n"
            "> Medido el 2026-08-14 en ROXs 12 b: a 7500 Å el cruce se va de 18.2 a 8.7 λ/D "
            "entre `C = 0` y `C = 4·C_fit`, **pero el núcleo pasa de +2 % a +42 %**. `C` mueve "
            "la frontera y no la cierra, y el motivo es geométrico: el corte de la AO cae en "
            "8.5–16.5 px y `norm_radius_px` es 25, así que el déficit está **dentro** de la "
            "normalización y todo lo que lo rellene mueve la `apcorr`."
        ),
        code(
            "if ES_PSFAO:\n"
            "    from maoppy.psfmodel import Psfao as _PsfaoC\n"
            "    from musepipe.psf import core_to_norm_ratio as _razonC\n\n"
            "    def _amp_bck(_d, _p, _wt, _m):\n"
            "        \"\"\"(a, b) que minimizan sum w (a·P + b − D)^2 sobre la máscara.\"\"\"\n"
            "        _dd, _pp, _ww = _d[_m].ravel(), _p[_m].ravel(), _wt[_m].ravel()\n"
            "        _o = np.isfinite(_dd) & np.isfinite(_pp) & np.isfinite(_ww)\n"
            "        _dd, _pp, _ww = _dd[_o], _pp[_o], _ww[_o]\n"
            "        _spp = float(np.sum(_ww * _pp * _pp)); _sp = float(np.sum(_ww * _pp))\n"
            "        _sw = float(np.sum(_ww)); _spd = float(np.sum(_ww * _pp * _dd))\n"
            "        _sd = float(np.sum(_ww * _dd)); _det = _spp * _sw - _sp * _sp\n"
            "        if abs(_det) < 1e-30:\n"
            "            return (float(_spd / _spp) if _spp else 0.0), 0.0\n"
            "        return (_spd * _sw - _sp * _sd) / _det, (_spp * _sd - _sp * _spd) / _det\n\n"
            "    _iC = NOMBRES.index('C')\n"
            "    for _k in IDX_BINS[::max(1, len(IDX_BINS) // 3)][:3]:\n"
            "        _a0, _b0, _mid, _sel = BINS_PSFAO[_k]\n"
            "        _sub = np.zeros(CANALES_SUB.size, dtype=bool)\n"
            "        _sub[[_pos[int(_c)] for _c in np.where(_sel)[0]]] = True\n"
            "        _img = np.nanmedian(CUBE_SUB[_sub], axis=0)\n"
            "        _var = np.nanmedian(STAT_SUB[_sub], axis=0)\n"
            "        _fila = next((_f for _f in FILAS\n"
            "                      if abs(float(_f['lambda_A']) - _mid) < 1e-6), None)\n"
            "        if _fila is None:\n"
            "            continue\n"
            "        _ny, _nx = _img.shape\n"
            "        _yy2, _xx2 = np.mgrid[0:_ny, 0:_nx]\n"
            "        _rr2 = np.hypot(_yy2 - _ny // 2, _xx2 - _nx // 2)\n"
            "        _msk = np.hypot(_yy2 - COMP_YX[0], _xx2 - COMP_YX[1]) <= MASK_RADIUS_PX\n"
            "        if FIELD_YX is not None:\n"
            "            _msk |= np.hypot(_yy2 - FIELD_YX[0], _xx2 - FIELD_YX[1]) <= MASK_RADIUS_PX\n"
            "        _w2 = psfao_fit_weights(_img, _var, _rr2, PESOS_AJUSTE, cap=TOPE_PESOS)\n"
            "        _aj = (_rr2 <= FIT_RADIUS_PX) & ~_msk\n"
            "        _samp2 = float(SYSTEM.samp(_mid * 1e-10))\n"
            "        _x0c = [float(_fila[_n]) for _n in NOMBRES]\n"
            "        _mod2 = _PsfaoC((_ny, _nx), system=SYSTEM, samp=_samp2)\n"
            "        _centro = (_ny // 2, _nx // 2)\n"
            "        _dato = _razonC(_img, _centro, norm_radius_px=NORM_RADIUS_PX,\n"
            "                        background=corner_background(_img), exclude_mask=_msk)\n"
            "        print(f'λ = {_mid:.0f} Å · r_AO = {0.5 * SYSTEM.Nact * _samp2:.1f} px'\n"
            "              f'  ·  C ajustado = {_x0c[_iC]:.4g}')\n"
            "        print(f'{\"C/C_fit\":>9s} {\"cruce λ/D\":>10s} {\"núcleo vs dato\":>15s}')\n"
            "        for _f2 in (0.0, 1.0, 2.0, 4.0):\n"
            "            _xc = list(_x0c); _xc[_iC] = _x0c[_iC] * _f2\n"
            "            with _w.catch_warnings():\n"
            "                _w.simplefilter('ignore')\n"
            "                _psf2 = np.asarray(_mod2(_xc, dx=float(_fila.get('dx', 0.0)),\n"
            "                                         dy=float(_fila.get('dy', 0.0))), float)\n"
            "            _a2, _b2 = _amp_bck(_img, _psf2, _w2, _aj)\n"
            "            _m2 = _a2 * _psf2 + _b2\n"
            "            _rd, _pr = radial_hybrid_profile(_img - _m2, _centro, mask=_msk,\n"
            "                                            smoothing_scale_px=0.0)\n"
            "            _rd2, _pe = radial_hybrid_profile(_img, _centro, mask=_msk,\n"
            "                                             smoothing_scale_px=0.0)\n"
            "            with np.errstate(invalid='ignore', divide='ignore'):\n"
            "                _rel2 = _pr / np.abs(_pe)\n"
            "            _v2 = np.where((_rd >= 5.0) & (_rd <= 30.0), _rel2, np.nan)\n"
            "            _ii = np.where(np.isfinite(_v2))[0]\n"
            "            _cr = np.nan\n"
            "            if _ii.size >= 3:\n"
            "                _pk = _ii[int(np.nanargmax(_v2[_ii]))]\n"
            "                for _q in range(_pk, _ii[-1]):\n"
            "                    if (np.isfinite(_v2[_q]) and np.isfinite(_v2[_q + 1])\n"
            "                            and _v2[_q] > 0 >= _v2[_q + 1]):\n"
            "                        _t2 = _v2[_q] / (_v2[_q] - _v2[_q + 1])\n"
            "                        _cr = float(_rd[_q] + _t2 * (_rd[_q + 1] - _rd[_q]))\n"
            "                        break\n"
            "            _nuc = _razonC(_m2, _centro, norm_radius_px=NORM_RADIUS_PX,\n"
            "                           background=_b2, exclude_mask=_msk)\n"
            "            print(f'{_f2:9.2g} {_cr / _samp2:10.1f} {100 * (_nuc / _dato - 1):+14.0f}%')\n"
            "        print()\n"
            "else:\n"
            "    print('esta sección es de la forma psfao; en Moffat no hay `C`.')"
        ),
        md(
            "**Cómo leerlo.** Congelar `C` **mejora** si el residuo de anillo baja y el coste "
            "sube menos de un uno por ciento; **empeora** si el precio se concentra en algún "
            "bin concreto. Las dos cosas pueden pasar a la vez, y entonces lo que hay que mirar "
            "es *dónde*: un parámetro que sólo trabaja en tres bins no está describiendo la PSF, "
            "está tapando un defecto local — y conviene ir a ver qué hay en esos bins (bandas "
            "telúricas de agua, fringing, un segundo mínimo del ajuste) antes de darle sentido "
            "físico.\n\n"
            "Y comparar las dos maneras de congelarlo: fijar `C` a la **mediana** le impone un "
            "pedestal a los bins que eligieron cero por su cuenta, mientras que `C = 0` sólo "
            "les quita libertad a los que sí lo usaban. Si el azul empeora con la mediana y no "
            "con el cero, es que ahí el dato nunca pidió pedestal.\n\n"
            "> Ojo con el cromatismo: es de esperar que **no se mueva**. `C` es un pedestal "
            "ancho y casi plano, y el cociente `F(≤32)/F(≤25)` de la §13.b apenas lo nota. Que "
            "no se mueva no dice que `C` esté bien; dice que el problema del halo no está aquí."
        ),
        md(
            "## 14 · Qué se lleva uno de aquí\n\n"
            "Para un run con la forma **psfao**:\n\n"
            "1. **El `polynomial_deg2` del QC no es lo que evalúa la cadena.** Mientras exista "
            "`param_table`, `smoothed_poly` es letra muerta (§8, §12). Cualquier plan que "
            ...
            "5. **El híbrido no viaja con el modelo** (§11). El número que se publica y el que "
...
def build_apcorr_cells(mb, target, run_id):
    ...
    cells = [
        ...
        md(
            ...
            "Hasta aquí la cascada trata el psffit como una caja negra. Esta sección la abre, "
            "porque el cociente **psffit / apertura `R_NORM`** —las dos maneras de medir dentro "
            "del *mismo* radio, §8— no baja suave: cae **en bloques** y vuelve a subir.\n\n"
            "El ancho del bloque no es del cielo, es del código. `_evaluate_psfao` no evalúa la PSF "
        ...
        md(
            ...
            "Un escalón del cociente puede ser del dato o del modelo. Aquí se ve de quién: para los "
            "dos bloques con mayor salto se enseña, **antes y después**, el canal (mediana del "
            "bloque), el área que suma la apertura, el modelo del psffit y el residuo — y la resta "
            "de los tres.\n\n"
        ...
        md(
            ...
            "Mismo cubo, mismo ajuste, **a resolución completa** en una ventana estrecha, cambiando "
            "solo `psfao_wave_bin_A`:\n\n"
            "| perilla | qué hace |\n"
            "|---|---|\n"
            ...
            "Separa las dos hipótesis: si el culpable fuera el redondeo, `0` saldría plano. Si es la "
            "interpolación, `0` se hunde igual —solo que suave, y con los máximos encima de los "
            "bins— y el que sale plano es `100`."
        ),
        code(
        ...
        md(
            ...
            "El mismo ajuste de la §8, canal por canal, con la **otra** rejilla: 100 Å si la "
            "cadena va a 50, y 50 Å si el modelo ya lleva 100 escrito. El A/B vale en los dos "
            "sentidos; solo cambia qué columna es la buena.\n\n"
            "Evaluar en los bins no es una solución completa —en los huecos de `param_table` "
            "sigue interpolando a ciegas, y la **pendiente** en λ no la toca— pero mide cuánto "
            "de la deformación era sólo esto.\n\n"
        ...
        md(
            ...
            "4. **No cambia `psfao_wave_bin_A` en la cadena.** Las §§14–17 miden el escalón y "
            "enseñan que con `100` desaparece, pero esa perilla vive en `psf_model.json` (no hay "
            "knob en el config) y tocarla mueve **todos** los productos que consumen la PSF de C1 "
            "—C2, C3 `optimal_psfsub`, C4, la apcorr de E1—. Es decisión científica, y aquí solo "
            "está el número.\n"
            "5. **No separa los dos defectos más allá de medirlos.** Son dos apilados: el "
            "**escalón** de las §§14–17 (interpolar entre bins) y la **pendiente** de la §12 "
            "(el halo del modelo es poco cromático). El `100` borra el primero y deja el segundo "
            "intacto."
        ),
    ]
    return cells
...
def build_residuos_cells(mb, target, run_id):
    ...
    return [
        ...
        code(
            ...
            "# --- perillas de la §7.d (los mapas 2D, bin a bin) ---\n"
            "# Qué cubo se dibuja:\n"
            "#   0            el PRIMER cubo por exposición (el default);\n"
            "#   un entero    el i-ésimo de la lista que imprime la propia sección;\n"
            "#   una etiqueta ('08-29#01', 'exp03') ese cubo;\n"
            "#   'combinado'  el cubo de B2 sobre el que ajusta la cadena, y el ÚNICO\n"
            "#                caso en el que el modelo dibujado es exactamente el que\n"
            "#                C1 entregó (sin re-escalar nada).\n"
            "# Un objeto que no declare cubos por exposición cae en 'combinado' solo.\n"
            "CUBO_MAPAS = 0\n"
            "RADIO_VISTA_PX = 45.0       # medio lado del recorte de cada panel\n"
            "COLUMNAS_MAPAS = 4          # la rejilla es 4×N, con N = ceil(bins/4)\n"
            "PASO_BINS_MAPAS = 1         # 1 = todos los bins; 2 = uno de cada dos\n"
            "# `True` le quita a cada panel su propia mediana azimutal — o sea, todo lo\n"
            "# que la §7 ya enseña— y deja SÓLO lo que un promedio en ángulo no puede ver.\n"
            "# Útil cuando el residuo radial es tan grande que satura el color (pasa con\n"
            "# los cubos por exposición, cuya PSF no es la del combinado).\n"
        ...
        code(
            ...
            "    print('sin CSV de psfao: esta sección no aplica a la forma Moffat.')\n"
            "else:\n"
            "    # `or 0.0` y no un default: cuando la rama híbrida NO se aplica, el QC\n"
            "    # escribe la clave con `null`, y `float(None)` reventaba el notebook entero\n"
            "    # en el objeto que no lleva híbrido.\n"
            "    SUAVE_PX = float((QC_C1.get('hybrid') or {}).get('smoothing_scale_px') or 0.0)\n"
        ...
        md(
            ...
            "C1 ya calcula este residuo. La spec (§3.5) le llama **híbrido**: si el residuo del "
            "anillo del compañero supera el 5 % en más del 20 % de los bins, se le añade al "
            "modelo la **mediana azimutal del residuo**, suavizada, que es azimutalmente "
            "simétrica y por eso no puede absorber una fuente puntual. En este run está "
            "**aplicado**, y su producto es el `psf_hybrid_residual.fits` que la §5.d ya usó.\n\n"
            "Pero el término **no entra en `psf_model.json`**: el documento lleva una bandera "
            "`hybrid: true` que `_evaluate_psfao` no mira (§11 de `C1_chromatic_psf_debug`). "
            "O sea que C2–C6 evalúan un modelo **sin** él.\n\n"
            "La pregunta obvia era: **¿es éste el término que falta?** Si lo fuera, el "
            "problema del halo estaría medido y guardado desde el principio, y sólo faltaría "
            "hacerlo viajar. La sección lo mide en la región que importa."
        ),
        code(
        ...
        md(
            ...
            "`radial_hybrid_profile` toma la mediana azimutal por anillos de 1 px y luego la "
            "**suaviza con una gaussiana de σ ≥ 2×FWHM**. Esa escala es una salvaguarda "
            "deliberada de la spec —garantiza que el término no pueda absorber al compañero—, "
            "pero tiene un precio: a `R_NORM` la ventana toca radios muy interiores, y el "
            "residuo del **núcleo** es mucho mayor que el del halo.\n\n"
            "La celda compara el perfil tal cual sale (sin suavizar) con el que la cadena "
            "guarda. Si el suavizado infla el término varias veces en 25–32 px, el híbrido que "
            "hay en disco no es «el residuo del halo»: es el residuo del núcleo, esparcido."
        ),
        code(
        ...
        md(
            "## 7 · El mapa (radio, λ)\n\n"
            "La figura que resume todo: el residuo azimutal-mediano, **normalizado al perfil de "
            "la propia estrella** en cada bin para que los colores comparen forma y no brillo. "
            "Lo que se busca es estructura:\n\n"
            "- una franja **inclinada**, cuyo radio crece con λ → estructura ÓPTICA, y la "
            "candidata es el borde del área corregida por la AO, en `(Nact/2)·λ/D` = "
            "17 λ/D, o sea de 8.5 px en el azul a 16 px en el rojo. Medido el 2026-08-14 en "
            "este run: el residuo cambia de signo a **15.2 λ/D** (positivo dentro, +8 % del "
            "perfil; negativo fuera, −11 %), y el radio del cruce se mueve de 10.5 a 14.7 px "
            "entre 6300 y 9300 Å. **No son anillos de difracción**: MUSE NFM submuestrea "
            "(`samp` = 0.50–0.95 px por λ/D), y el primer anillo de Airy cae en 1.22 λ/D, "
            "dentro del píxel central. La difracción **sí está en el modelo** —maoppy "
            "multiplica la OTF de turbulencia por la de la pupila (D = 8 m, obstrucción "
            "0.14)—, pero no se resuelve;\n"
            "- una franja **vertical**, a radio FIJO en píxeles → no es ni AO ni difracción "
            "(las dos escalan con λ): mira al detector, al muestreo o a la máscara;\n"
            "- un **degradado en λ** al radio de la apcorr → el halo mal repartido, que es lo "
            "que la §5.c ya midió con números."
        ),
        code(
        ...
        md(
            "### 7.b · ¿Es el corte de la AO? La prueba está en cómo se mueve con λ\n\n"
            "La franja de la §7 recuerda a anillos, y hay dos candidatos ópticos. Los separa su "
            "**escala**, porque los dos van con λ pero a radios muy distintos:\n\n"
            "| | radio | a 7000 Å |\n"
            "|---|---|---|\n"
            "| difracción (primer anillo de Airy) | `1.22·λ/D` | **0.89 px** — dentro del píxel |\n"
            "| borde del área corregida por la AO | `(Nact/2)·λ/D` | **12.4 px** |\n\n"
            "MUSE NFM **submuestrea** (`samp` < 1 px por λ/D en toda la banda), así que la "
            "difracción no se resuelve por mucho que esté en el modelo —maoppy multiplica la OTF "
            "de turbulencia por la de la pupila (D = 8 m, obstrucción 0.14)—. El que sí se ve es "
            "el corte de la AO.\n\n"
            "La celda mide, **bin a bin y sin promediar**, el radio donde el residuo cambia de "
            "signo, y lo compara con `(Nact/2)·λ/D`. Tres desenlaces:\n\n"
            "- el cruce **sigue** a `r_AO` → es el borde del área corregida, y el modelo reparte "
            "mal la energía entre dentro y fuera;\n"
            "- el cruce está en un radio **fijo en píxeles** → no es óptico: detector, muestreo "
            "o máscara;\n"
            "- no hay cruce → el residuo no cambia de signo en ese tramo, que es lo que pasa en "
            "el azul de ROXs 12 b, donde `C` está pegado a cero y el modelo no tiene suelo del "
            "área corregida (traspaso del 2026-08-13 §3.a)."
        ),
        code(
            "if ES_PSFAO and _prod is not None:\n"
            "    from maoppy.instrument import muse_nfm as _sys_nfm\n\n"
            "    def _cruce(_rad, _perf, _lo=5.0, _hi=30.0):\n"
            "        \"\"\"Radio del primer cambio de + a - tras el máximo interior.\"\"\"\n"
            "        _v = np.where((_rad >= _lo) & (_rad <= _hi), _perf, np.nan)\n"
            "        _idx = np.where(np.isfinite(_v))[0]\n"
            "        if _idx.size < 3:\n"
            "            return np.nan\n"
            "        _pico = _idx[int(np.nanargmax(_v[_idx]))]\n"
            "        for _k in range(_pico, _idx[-1]):\n"
            "            if np.isfinite(_v[_k]) and np.isfinite(_v[_k + 1]) and _v[_k] > 0 >= _v[_k + 1]:\n"
            "                _t = _v[_k] / (_v[_k] - _v[_k + 1])\n"
            "                return float(_rad[_k] + _t * (_rad[_k + 1] - _rad[_k]))\n"
            "        return np.nan\n\n"
            "    print(f'{\"λ [Å]\":>7s} {\"samp\":>6s} {\"Airy 1º\":>8s} {\"r_AO\":>7s}'\n"
            "          f' {\"cruce px\":>9s} {\"cruce λ/D\":>10s}')\n"
            "    _lam_c, _rao_c, _cru_c = [], [], []\n"
            "    for _i, _w in enumerate(LAM_OK):\n"
            "        _samp = float(_sys_nfm.samp(float(_w) * 1e-10))\n"
            "        _rao = 0.5 * float(_sys_nfm.Nact) * _samp\n"
            "        _c = _cruce(RADIOS, _mapa[_i])\n"
            "        _lam_c.append(float(_w)); _rao_c.append(_rao); _cru_c.append(_c)\n"
            "        print(f'{_w:7.0f} {_samp:6.3f} {1.22 * _samp:8.2f} {_rao:7.1f}'\n"
            "              f' {_c:9.1f} {_c / _samp:10.1f}')\n"
            "    _lam_c = np.array(_lam_c); _rao_c = np.array(_rao_c); _cru_c = np.array(_cru_c)\n"
            "    _ok = np.isfinite(_cru_c)\n"
            "    if _ok.sum() >= 3:\n"
            "        _p_ao = np.polyfit(_lam_c[_ok], _rao_c[_ok], 1)[0] * 1e4\n"
            "        _p_ob = np.polyfit(_lam_c[_ok], _cru_c[_ok], 1)[0] * 1e4\n"
            "        print()\n"
            "        print(f'pendiente del radio con λ:  r_AO {_p_ao:+.2f} px/1000 Å'\n"
            "              f'  ·  medida {_p_ob:+.2f} px/1000 Å')\n"
            "        print(f'cruce mediano en {np.nanmedian(_cru_c[_ok] / (_rao_c[_ok] / (0.5 * _sys_nfm.Nact))):.1f}'\n"
            "              f' λ/D  (el corte de la AO está en {0.5 * _sys_nfm.Nact:.0f})')\n"
            "        print(f'sin cruce en {int((~_ok).sum())} de {_ok.size} bins')\n"
            "        fig, ax = plt.subplots(figsize=(7, 3.4))\n"
            "        ax.plot(_lam_c, _rao_c, '-', color='k', label='corte de la AO  (Nact/2)·λ/D')\n"
            "        ax.plot(_lam_c[_ok], _cru_c[_ok], 'o-', color='tab:red', ms=3,\n"
            "                label='cruce por cero del residuo')\n"
            "        ax.plot(_lam_c, 1.22 * _rao_c / (0.5 * _sys_nfm.Nact), ':', color='0.5',\n"
            "                label='primer anillo de Airy  1.22·λ/D')\n"
            "        ax.set_xlabel('λ [Å]'); ax.set_ylabel('radio [px]')\n"
            "        ax.legend(fontsize=7); ax.set_title('§7.b · la franja, ¿sigue a λ?', fontsize=9)\n"
            "        fig.tight_layout(); plt.show()\n"
            "else:\n"
            "    print('sin CSV de psfao: esta sección no aplica a la forma Moffat.')"
        ),
        md(
            "### 7.c · Y si el radio de normalización no fuera 25 px\n\n"
            "La `apcorr` que aplican C2/C3 es `F(≤R)/F(box3)` del modelo, con `R = norm_radius_px`. "
            "Si el residuo tiene déficit **dentro** del corte de la AO y exceso **fuera**, el error "
            "de ese cociente depende de R: cuanto más disco se coja por fuera del corte, más "
            "exceso acumula el modelo.\n\n"
            "La celda mide el error contra el DATO para una rejilla de R, con la misma función "
            "que C1 publica en su QC (`core_to_norm_ratio`, la V4 de la spec §7) — **importada, no "
            "copiada**: aquí no se audita esa función, se usa como vara.\n\n"
            "> Medido el 2026-08-14: la convención de 25 px cae **en el máximo** de la curva de "
            "error en los dos objetos (ROXs 12 b 8.3 %, ROXs 42B b 3.7 %) y el mínimo está en los "
            "dos en **R ≈ 8–10 px**, o sea dentro del área corregida. Bajarlo NO se recomienda sin "
            "más: el sistemático relativo de la curva de crecimiento no mejora (18 % a cualquier "
            "radio) y el factor empírico pasaría a llevar cuatro veces más cromatismo."
        ),
        code(
            "if ES_PSFAO and _prod is not None:\n"
            "    from musepipe.psf import core_to_norm_ratio as _razon\n"
            "    from musepipe.psf import corner_background as _fondo\n\n"
            "    _RADIOS_N = [5.0, 8.0, 10.0, 12.0, 15.0, 20.0, R_NORM, 30.0]\n"
            "    _err = []\n"
            "    for _img, _mod, _fila in zip(IMAGENES, MODELOS, FILAS):\n"
            "        _bkg = _fondo(_img)\n"
            "        _fil = []\n"
            "        for _R in _RADIOS_N:\n"
            "            _d = _razon(_img, STAR_YX, norm_radius_px=_R, background=_bkg,\n"
            "                        exclude_mask=MASCARA_COMP)\n"
            "            _m = _razon(_mod, STAR_YX, norm_radius_px=_R,\n"
            "                        background=float(_fila['bck']), exclude_mask=MASCARA_COMP)\n"
            "            _fil.append(100.0 * (_m / _d - 1.0))\n"
            "        _err.append(_fil)\n"
            "    _err = np.array(_err)\n"
            "    _az = LAM_OK < np.median(LAM_OK)\n"
            "    print(f'{\"R [px]\":>7s} {\"|error| mediano\":>16s} {\"azul − rojo\":>12s}')\n"
            "    for _j, _R in enumerate(_RADIOS_N):\n"
            "        _marca = '   <- la convención' if abs(_R - R_NORM) < 1e-6 else ''\n"
            "        print(f'{_R:7.1f} {np.nanmedian(np.abs(_err[:, _j])):15.1f}%'\n"
            "              f' {np.nanmedian(_err[_az, _j]) - np.nanmedian(_err[~_az, _j]):+11.1f}%{_marca}')\n"
            "    _mejor = _RADIOS_N[int(np.nanargmin([np.nanmedian(np.abs(_err[:, _j]))\n"
            "                                         for _j in range(len(_RADIOS_N))]))]\n"
            "    print(f'\\nmínimo del error en R = {_mejor:.0f} px'\n"
            "          f'  (la convención está en {R_NORM:.0f})')\n"
            "else:\n"
            "    print('sin CSV de psfao: esta sección no aplica a la forma Moffat.')"
        ),
        md(
            "### 7.d · El residuo sin colapsar: una rejilla 4×N, bin a bin\n\n"
            "Todo lo anterior **colapsa** el residuo: la §5.d y la §7 lo promedian en ángulo, la "
            "§5.c lo suma por regiones. Un promedio azimutal no puede enseñar un lóbulo, un "
            "dipolo ni una franja rota, y esas son justo las firmas que separan «al modelo le "
            "falta halo» de «al modelo le falta *forma*». Esta sección enseña el residuo tal "
            "cual, **un panel por bin de λ**, en una rejilla de `COLUMNAS_MAPAS`×N con N los que "
            "hagan falta.\n\n"
            "Cada panel es `(dato − modelo) / |perfil de la propia estrella|`, **la misma "
            "normalización de la §7** — así que la mediana azimutal de un panel es una fila del "
            "mapa de la §7, y las dos figuras se leen con la misma escala de color: rojo = al "
            "modelo le falta luz, azul = le sobra. Encima van dibujados el radio de "
            "normalización (punteado), el **corte de la AO** `(Nact/2)·λ/D` de ese λ (a trazos) "
            "y la posición del compañero.\n\n"
            "Qué se busca, y qué significaría:\n\n"
            "| lo que se vea recorriendo los paneles | lo que significa |\n|---|---|\n"
            "| el anillo **sigue** al círculo a trazos | es el corte de la AO — la §7.b, ya sin promediar |\n"
            "| el anillo se queda **a radio fijo** en px | ni AO ni difracción: detector, muestreo o máscara |\n"
            "| **dos lóbulos opuestos** (dipolo) | ninguna elipse puede producirlo: es el suelo del anillo medido en ROXs 42B b (23 % al rojo) |\n"
            "| cuatro lóbulos alineados con el eje mayor | elipticidad mal ajustada — del ajuste, no del cielo |\n"
            "| tinte uniforme de todo el panel | fondo mal restado en ese bin |\n\n"
            "Cuando el residuo **radial** es tan grande que satura el color —le pasa a los cubos "
            "por exposición, cuya PSF no es la del combinado— `RESTA_PERFIL_AZIMUTAL = True` le "
            "quita a cada panel su propia mediana azimutal, o sea **todo lo que la §7 ya "
            "enseña**, y deja sólo lo que un promedio en ángulo no puede ver. La celda avisa "
            "cuando conviene.\n\n"
            "**Qué cubo se dibuja.** Por defecto el **primer cubo por exposición** de los que el "
            "config declara (`perexp_cubes` / `perexp_dir`, nunca por convención de nombres "
            "sobre un disco), y `CUBO_MAPAS` admite el índice, la etiqueta, o **`'combinado'`** "
            "para el cubo de B2 que la cadena ajusta. La diferencia no es cosmética:\n\n"
            "- con **`'combinado'`** el modelo es *exactamente* el que C1 entregó — los mismos "
            "`amp`, `bck`, `dx`, `dy` del CSV— y el panel es la §7 sin colapsar;\n"
            "- con un **cubo por exposición** aquí **se sigue sin ajustar nada**: dato y modelo "
            "se llevan a la **misma energía dentro de `R_NORM`** (al dato se le quita antes su "
            "`corner_background`), que es exactamente como la cadena normaliza la PSF. Una "
            "exposición no tiene el tiempo ni la transmisión del combinado, y un factor común no "
            "cambia el mapa —el residuo se divide por el perfil del propio dato—, así que lo "
            "único que se decide es el peso **relativo** modelo/dato, y se decide con la "
            "convención de la cadena y no con un ajuste. Lo que queda a la vista es **cómo "
            "reparte cada uno esa misma energía** entre el núcleo y el halo. La ventana se "
            "centra en la primaria de *esa* exposición a píxel entero (≤ 0.5 px de resto, la "
            "receta de C1 §10 y D2), y ese medio píxel deja un **dipolo en el núcleo que es del "
            "centrado, no del modelo** — el único artefacto que hay que descontar al leer los "
            "paneles.\n\n"
            "> Con una exposición se imprime además la **V4 de la spec** —`F(≤R_NORM)/F(box3)` "
            "del modelo contra la del dato, que es la corrección de apertura hecha cantidad "
            "medible— sobre ese cubo concreto: la cadena sólo la mide sobre el combinado.\n\n"
            "> Un cubo por exposición se lee **por bin y submuestreado** "
            "(`CANALES_POR_BIN_MAPAS` canales de cada uno): para una figura la mediana de una "
            "docena de canales sobra, y leer los 3681 costaría cientos de MB por panel."
        ),
        code(
            "if not ES_PSFAO:\n"
            "    print('sin CSV de psfao: esta sección no aplica a la forma Moffat.')\n"
            "else:\n"
            "    from maoppy.psfmodel import Psfao as _Ps\n"
            "    # Importada, no copiada: reconstruir el eje λ de una cabecera es de la\n"
            "    # cadena de reducción y aquí se da por bueno. Los cubos por exposición no\n"
            "    # traen extensión WAVELENGTH, que sí trae el de B2.\n"
            "    from musepipe.reduction.telluric import wavelength_axis_from_header as _eje_lambda\n"
            "    # Las mismas dos varas de la §7.c, importadas y no copiadas: aquí no se\n"
            "    # auditan, se usan para poner dato y modelo en la misma escala.\n"
            "    from musepipe.psf import core_to_norm_ratio, corner_background\n\n"
            "    SIS_MAPAS = INP['system']\n"
            "    CFG_RUN = json.loads((RD / 'config' / 'config.json')\n"
            "                         .read_text(encoding='utf-8'))['config']\n\n"
            "    def _etiquetas_de(rutas):\n"
            "        \"\"\"Un nombre corto y ÚNICO por cubo, sacado de su DIRECTORIO.\n\n"
            "        `p.stem` daría `DATACUBE_FINAL` para todos. Misma receta que C1 §10\n"
            "        y D2, porque las etiquetas tienen que ser las mismas en los tres.\n"
            "        \"\"\"\n"
            "        crudas = [p.parent.name for p in rutas]\n"
            "        if all(c.lower().startswith('exp') for c in crudas) and len(set(crudas)) == len(crudas):\n"
            "            return crudas\n"
            ...
            "            salida.append(f'{noche}#{cuenta[noche]:02d}')\n"
            "        return salida if len(set(salida)) == len(salida) else crudas\n\n"
            "    def _cubos_declarados():\n"
            "        \"\"\"Los cubos por exposición que el CONFIG declara, y que existen.\"\"\"\n"
            "        lista = CFG_RUN.get('perexp_cubes') or nb._sibling_setting(RUN_ID, 'perexp_cubes')\n"
            "        if not lista:\n"
            "            raiz = CFG_RUN.get('perexp_dir') or nb._sibling_setting(RUN_ID, 'perexp_dir')\n"
            "            if raiz:\n"
            "                lista = sorted(str(p) for p in Path(raiz).glob('*/DATACUBE_FINAL.fits'))\n"
            "        rutas = [Path(p) for p in (lista or []) if Path(p).exists()]\n"
            "        return list(zip(_etiquetas_de(rutas), rutas))\n\n"
            "    OBS_MAPAS = _cubos_declarados()\n"
            "    _pedido = CUBO_MAPAS\n"
            "    ES_COMBINADO = (isinstance(_pedido, str)\n"
            "                    and _pedido.strip().lower() in ('combinado', 'combined', 'stack'))\n"
            "    if not ES_COMBINADO and not OBS_MAPAS:\n"
            "        print('este objeto no declara cubos por exposición (`perexp_cubes` /'\n"
            "              ' `perexp_dir`):')\n"
            "        print('se dibuja el COMBINADO, que es el que la cadena ajusta. Para tenerlos,')\n"
            "        print('   python scripts/regen_perexp_cubes.py --step all')\n"
            "        ES_COMBINADO = True\n"
            "    if ES_COMBINADO:\n"
            "        ETQ_MAPAS, RUTA_MAPAS = 'combinado (el cubo de B2)', None\n"
            "    else:\n"
            "        if isinstance(_pedido, str):\n"
            "            _cand = [k for k, (e, _p) in enumerate(OBS_MAPAS) if e == _pedido]\n"
            "            if not _cand:\n"
            "                print(f'no hay ningún cubo con la etiqueta {_pedido!r}. Los hay:')\n"
            "                print('   ' + ', '.join(e for e, _p in OBS_MAPAS))\n"
            "                print('se dibuja el primero.')\n"
            "            _k = _cand[0] if _cand else 0\n"
            "        else:\n"
            "            _k = int(_pedido)\n"
            "            if not 0 <= _k < len(OBS_MAPAS):\n"
            "                print(f'CUBO_MAPAS = {_k} fuera de rango (hay {len(OBS_MAPAS)}):'\n"
            "                      ' se dibuja el primero.')\n"
            "                _k = 0\n"
            "        ETQ_MAPAS, RUTA_MAPAS = OBS_MAPAS[_k]\n"
            "        print(f'{len(OBS_MAPAS)} cubos por exposición declarados'\n"
            "              f'  ({\", \".join(e for e, _p in OBS_MAPAS[:6])}'\n"
            "              f'{\", …\" if len(OBS_MAPAS) > 6 else \"\"})')\n"
            "    print(f'cubo dibujado: {ETQ_MAPAS}')\n\n"
            "    def _energia(arr, donde):\n"
            "        \"\"\"Energía encapsulada dentro de `donde`, ignorando NaN.\"\"\"\n"
            "        return float(np.nansum(np.where(donde, arr, 0.0)))\n\n"
            "    def _de_una_exposicion(ruta):\n"
            "        \"\"\"Imágenes y modelos por bin de un cubo por exposición.\"\"\"\n"
            "        _ny, _nx = IMAGENES[0].shape\n"
            "        with fits.open(ruta, memmap=True) as _h:\n"
            "            _hdu = _h['DATA'] if 'DATA' in _h else _h[1]\n"
            "            # La primaria de ESTA exposición: máximo + centroide de la imagen\n"
            "            # blanca. Los bordes son NaN en todos los canales y `nanmedian`\n"
            "            # avisa por cada columna vacía; es esperado.\n"
            "            with warnings.catch_warnings():\n"
            "                warnings.simplefilter('ignore', RuntimeWarning)\n"
            "                _blanco = np.nanmedian(np.asarray(_hdu.data[::37], dtype=float), axis=0)\n"
            "            _py, _px = (int(v) for v in np.unravel_index(np.nanargmax(_blanco), _blanco.shape))\n"
            "            _sy = slice(max(0, _py - 3), _py + 4); _sx = slice(max(0, _px - 3), _px + 4)\n"
            "            _caja = np.nan_to_num(_blanco[_sy, _sx])\n"
            "            _yc, _xc = np.mgrid[_sy.start:_sy.start + _caja.shape[0],\n"
            "                                _sx.start:_sx.start + _caja.shape[1]]\n"
            "            _cy = float((_yc * _caja).sum() / _caja.sum())\n"
            "            _cx = float((_xc * _caja).sum() / _caja.sum())\n"
            "            _nyc, _nxc = _hdu.shape[1:]\n"
            "            _y0 = int(np.clip(round(_cy) - _ny // 2, 0, max(0, _nyc - _ny)))\n"
            "            _x0 = int(np.clip(round(_cx) - _nx // 2, 0, max(0, _nxc - _nx)))\n"
            "            _ys, _xs = slice(_y0, _y0 + _ny), slice(_x0, _x0 + _nx)\n"
            "            # La ventana deja la primaria en el centro del array, que es donde\n"
            "            # `fit_bin` la da por hecha — y por eso el modelo del CSV vale aquí.\n"
            "            _forma_v = (min(_ny, _nyc - _y0), min(_nx, _nxc - _x0))\n"
            "            _cen = (_forma_v[0] // 2, _forma_v[1] // 2)\n"
            "            # El compañero, respecto del CENTRO DEL ARRAY: es lo que la cadena\n"
            "            # usa de verdad para enmascarar, y el DRS deja todas las\n"
            "            # exposiciones con la misma orientación.\n"
            "            _comp = (_cen[0] + COMP_YX[0] - IMAGENES[0].shape[0] // 2,\n"
            "                     _cen[1] + COMP_YX[1] - IMAGENES[0].shape[1] // 2)\n"
            "            _msk = source_mask(_forma_v, [_comp], MASK_RADIUS_PX)\n"
            "            _yy2, _xx2 = np.mgrid[0:_forma_v[0], 0:_forma_v[1]]\n"
            "            # Los píxeles con los que se iguala la energía: los mismos que\n"
            "            # normalizan la PSF en la cadena (`norm_radius_px`), sin el compañero.\n"
            "            _donde = (np.hypot(_yy2 - _cen[0], _xx2 - _cen[1]) <= R_NORM) & (~_msk)\n"
            "            _wave = _eje_lambda(_hdu.header, int(_hdu.shape[0]))\n"
            "            # Los bins, sobre el eje λ de ESTE cubo y con el binado de la\n"
            "            # cadena (del config resuelto, no literales).\n"
            "            _bins = make_bins(_wave, float(INP['bin_A']), INP['bad'],\n"
            "                              int(E01.get('psf_min_channels_per_bin', 3)))\n"
            "            _centros = np.array([float(b[2]) for b in _bins]) if _bins else np.array([])\n"
            "            _imgs, _mods, _lams, _razones = [], [], [], []\n"
            "            for _fila in FILAS:\n"
            "                _lam = float(_fila['lambda_A'])\n"
            "                if not _centros.size:\n"
            "                    break\n"
            "                _j = int(np.argmin(np.abs(_centros - _lam)))\n"
            "                if abs(_centros[_j] - _lam) > 0.5 * float(INP['bin_A']):\n"
            "                    continue\n"
            "                _idx = np.where(_bins[_j][3])[0]\n"
            "                _paso = max(1, int(np.ceil(_idx.size / max(1, int(CANALES_POR_BIN_MAPAS)))))\n"
            "                with warnings.catch_warnings():\n"
            "                    warnings.simplefilter('ignore', RuntimeWarning)\n"
            "                    _img = np.nanmedian(np.asarray(_hdu.data[_idx[::_paso], _ys, _xs],\n"
            "                                                   dtype=float), axis=0)\n"
            "                _m = _Ps(_img.shape, system=SIS_MAPAS, samp=float(_fila['samp']))\n"
            "                with warnings.catch_warnings():\n"
            "                    warnings.simplefilter('ignore')\n"
            "                    _psf = float(_fila['amp']) * _m(\n"
            "                        [float(_fila[n]) for n in NOMBRES],\n"
            "                        dx=float(_fila['dx']), dy=float(_fila['dy']))\n"
            "                # Nada de ajustar: dato y modelo se llevan a la MISMA energía\n"
            "                # dentro de R_NORM, que es la normalización con la que la\n"
            "                # cadena usa la PSF. Un factor común no cambia el mapa (el\n"
            "                # residuo se divide por el perfil del propio dato), así que lo\n"
            "                # único que se decide aquí es el peso RELATIVO modelo/dato.\n"
            "                _fondo_e = corner_background(_img)\n"
            "                _dat = _img - _fondo_e\n"
            "                _e_d, _e_m = _energia(_dat, _donde), _energia(_psf, _donde)\n"
            "                if not (_e_d > 0 and _e_m > 0):\n"
            "                    continue\n"
            "                _imgs.append(_dat / _e_d); _mods.append(_psf / _e_m)\n"
            "                _lams.append(_lam)\n"
            "                # La V4 de la spec C1 sobre ESTA exposición: `F(<=R_NORM)/F(box3)`\n"
            "                # del modelo contra el del dato, que es la corrección de\n"
            "                # apertura hecha cantidad medible.\n"
            "                _razones.append((\n"
            "                    core_to_norm_ratio(_img, _cen, norm_radius_px=R_NORM,\n"
            "                                       background=_fondo_e, exclude_mask=_msk),\n"
            "                    core_to_norm_ratio(_psf, _cen, norm_radius_px=R_NORM,\n"
            "                                       background=0.0, exclude_mask=_msk)))\n"
            "        return (_imgs, _mods, np.array(_lams), _cen, _msk,\n"
            "                np.array(_razones), (_cy - _y0, _cx - _x0))\n\n"
            "    if ES_COMBINADO:\n"
            "        IMG_MAPAS, MOD_MAPAS = IMAGENES, MODELOS\n"
            "        LAM_MAPAS, CENTRO_MAPAS, MSK_MAPAS = LAM_OK, STAR_YX, MASCARA_COMP\n"
            "        RAZONES_MAPAS, _resto_px = np.empty((0, 2)), (0.0, 0.0)\n"
            "    else:\n"
            "        (IMG_MAPAS, MOD_MAPAS, LAM_MAPAS, CENTRO_MAPAS, MSK_MAPAS,\n"
            "         RAZONES_MAPAS, _prim_px) = _de_una_exposicion(RUTA_MAPAS)\n"
            "        _resto_px = (_prim_px[0] - CENTRO_MAPAS[0], _prim_px[1] - CENTRO_MAPAS[1])\n"
            "        if RAZONES_MAPAS.size:\n"
            "            _v4 = RAZONES_MAPAS[:, 1] / RAZONES_MAPAS[:, 0]\n"
            "            print(f'V4 sobre esta exposición — F(≤{R_NORM:.0f})/F(box3) del modelo'\n"
            "                  f' contra el del dato: {np.median(_v4 - 1):+.1%}'\n"
            "                  f'  (recorrido {np.min(_v4 - 1):+.1%}..{np.max(_v4 - 1):+.1%})')\n"
            "            print(f'resto del centrado a píxel entero: '\n"
            "                  f'({_resto_px[0]:+.2f}, {_resto_px[1]:+.2f}) px'\n"
            "                  '  <- el dipolo del núcleo es esto, no el modelo')\n"
            "    print(f'{len(IMG_MAPAS)} bins con imagen y modelo')"
        ),
        code(
            "if ES_PSFAO and len(IMG_MAPAS):\n"
            "    _paso_b = max(1, int(PASO_BINS_MAPAS))\n"
            "    _cuales = list(range(0, len(LAM_MAPAS), _paso_b))\n"
            "    _cy0, _cx0 = float(CENTRO_MAPAS[0]), float(CENTRO_MAPAS[1])\n"
            "    _ry0 = int(max(0, round(_cy0 - RADIO_VISTA_PX)))\n"
            "    _ry1 = int(min(IMG_MAPAS[0].shape[0], round(_cy0 + RADIO_VISTA_PX) + 1))\n"
            "    _rx0 = int(max(0, round(_cx0 - RADIO_VISTA_PX)))\n"
            "    _rx1 = int(min(IMG_MAPAS[0].shape[1], round(_cx0 + RADIO_VISTA_PX) + 1))\n"
            "    # La misma normalización de la §7: el residuo, en fracción del perfil de\n"
            "    # la PROPIA estrella, para que los colores comparen forma y no brillo.\n"
            "    MAPAS_2D = []\n"
            "    for _k in _cuales:\n"
            "        _rad_k, _per_k = radial_hybrid_profile(IMG_MAPAS[_k], CENTRO_MAPAS,\n"
            "                                              mask=MSK_MAPAS, smoothing_scale_px=0.0)\n"
            "        _per2d = evaluate_radial_profile(IMG_MAPAS[_k].shape, CENTRO_MAPAS,\n"
            "                                        _rad_k, np.abs(_per_k))\n"
            "        with np.errstate(invalid='ignore', divide='ignore'):\n"
            "            _mp = (IMG_MAPAS[_k] - MOD_MAPAS[_k]) / _per2d\n"
            "        if RESTA_PERFIL_AZIMUTAL:\n"
            "            _r_m, _p_m = radial_hybrid_profile(_mp, CENTRO_MAPAS, mask=MSK_MAPAS,\n"
            "                                              smoothing_scale_px=0.0)\n"
            "            _mp = _mp - evaluate_radial_profile(_mp.shape, CENTRO_MAPAS, _r_m, _p_m)\n"
            "        MAPAS_2D.append(_mp[_ry0:_ry1, _rx0:_rx1])\n"
            "    _VM = float(np.nanpercentile(np.abs(np.array(MAPAS_2D)), 98))\n"
            "    _cols = max(1, int(COLUMNAS_MAPAS))\n"
            "    _fils = int(np.ceil(len(MAPAS_2D) / _cols))\n"
            "    fig, ejes = plt.subplots(_fils, _cols, figsize=(2.5 * _cols, 2.65 * _fils),\n"
            "                             squeeze=False)\n"
            "    _ext = [_rx0 - _cx0, _rx1 - 1 - _cx0, _ry0 - _cy0, _ry1 - 1 - _cy0]\n"
            "    _dy_c = COMP_YX[0] - STAR_YX[0]\n"
            "    _dx_c = COMP_YX[1] - STAR_YX[1]\n"
            "    _dib = None\n"
            "    for _n, _k in enumerate(_cuales):\n"
            "        ax = ejes[_n // _cols][_n % _cols]\n"
            "        _dib = ax.imshow(MAPAS_2D[_n], origin='lower', extent=_ext, cmap='RdBu_r',\n"
            "                         vmin=-_VM, vmax=_VM, interpolation='nearest')\n"
            "        _samp_k = float(SIS_MAPAS.samp(float(LAM_MAPAS[_k]) * 1e-10))\n"
            "        ax.add_patch(plt.Circle((0, 0), R_NORM, fill=False, color='k',\n"
            "                                lw=0.6, ls=':'))\n"
            "        ax.add_patch(plt.Circle((0, 0), 0.5 * float(SIS_MAPAS.Nact) * _samp_k,\n"
            "                                fill=False, color='k', lw=0.7, ls='--'))\n"
            "        if abs(_dy_c) < RADIO_VISTA_PX and abs(_dx_c) < RADIO_VISTA_PX:\n"
            "            ax.plot([_dx_c], [_dy_c], marker='o', ms=6, mfc='none',\n"
            "                    mec='0.2', mew=0.8)\n"
            "        ax.set_title(f'{LAM_MAPAS[_k]:.0f} Å', fontsize=7)\n"
            "        ax.set_xticks([]); ax.set_yticks([]); ax.set_box_aspect(1)\n"
            "    for _n in range(len(MAPAS_2D), _fils * _cols):\n"
            "        ejes[_n // _cols][_n % _cols].axis('off')\n"
            "    fig.suptitle(f'§7.d · residuo / perfil de la estrella · {ETQ_MAPAS}'\n"
            "                 f'{\" · sin su mediana azimutal\" if RESTA_PERFIL_AZIMUTAL else \"\"}'\n"
            "                 '   (rojo = al modelo le falta luz; azul = le sobra)\\n'\n"
            "                 f'punteado: R_NORM = {R_NORM:.0f} px  ·  trazos: corte de la AO'\n"
            "                 ' (Nact/2)·λ/D  ·  círculo: compañero', fontsize=9)\n"
            "    fig.tight_layout(rect=(0, 0, 0.92, 0.97))\n"
            "    fig.colorbar(_dib, ax=ejes.ravel().tolist(), fraction=0.02, pad=0.01,\n"
            "                 label='fracción del perfil')\n"
            "    plt.show()\n"
            "    _signo = float(np.nanmean(np.sign(np.array(MAPAS_2D))))\n"
            "    if not RESTA_PERFIL_AZIMUTAL and abs(_signo) > 0.6:\n"
            "        print(f'{50 * (1 + abs(_signo)):.0f} % de los píxeles tienen el mismo signo:'\n"
            "              ' el término radial domina')\n"
            "        print('el color. `RESTA_PERFIL_AZIMUTAL = True` se lo quita a cada panel y'\n"
            "              ' deja sólo')\n"
            "        print('la estructura que un promedio en ángulo —la §7— no puede enseñar.')\n"
            "    # Y los mismos paneles, en número: el residuo por regiones y por tramos\n"
            "    # de λ. Con 'combinado' la columna del anillo tiene que ser la de la §5.c;\n"
            "    # con una exposición, `r<=R_NORM` sale ~0 POR CONSTRUCCIÓN (es donde se\n"
            "    # igualó la energía), y lo que informa son las otras dos.\n"
            "    _yyv, _xxv = np.mgrid[0:IMG_MAPAS[0].shape[0], 0:IMG_MAPAS[0].shape[1]]\n"
            "    _rrv = np.hypot(_yyv - _cy0, _xxv - _cx0)\n"
            "    _REG = ((f'r<={R_NORM:.0f}', _rrv <= R_NORM),\n"
            "            ('r<=5', _rrv <= 5.0),\n"
            "            (f'{R_NORM:.0f}-{R_GRANDE:.0f}', (_rrv > R_NORM) & (_rrv <= R_GRANDE)),\n"
            "            (f'{R_GRANDE:.0f}-60', (_rrv > R_GRANDE) & (_rrv <= 60.0)))\n"
            "    _REG = [(_et, _s & (~MSK_MAPAS)) for _et, _s in _REG]\n"
            "    _lam_v = np.array([LAM_MAPAS[_k] for _k in _cuales])\n"
            "    print()\n"
            "    print('residuo (dato − modelo) en fracción del dato de cada región:')\n"
            "    print(f'   {\"tramo\":16s} {\"paneles\":>8s}'\n"
            "          + ''.join(f'{_et:>12s}' for _et, _s in _REG))\n"
            "    for _et, _cual in ((f'azul  <{CORTE_AZUL_A:.0f}', _lam_v < CORTE_AZUL_A),\n"
            "                       ('medio', (_lam_v >= CORTE_AZUL_A) & (_lam_v <= CORTE_ROJO_A)),\n"
            "                       (f'rojo  >{CORTE_ROJO_A:.0f}', _lam_v > CORTE_ROJO_A)):\n"
            "        if not _cual.any():\n"
            "            continue\n"
            "        _vals = [[] for _s in _REG]\n"
            "        for _n, _k in enumerate(_cuales):\n"
            "            if not _cual[_n]:\n"
            "                continue\n"
            "            _d, _m = IMG_MAPAS[_k], MOD_MAPAS[_k]\n"
            "            for _c, (_e, _s) in enumerate(_REG):\n"
            "                _vals[_c].append(float(np.nansum((_d - _m)[_s])\n"
            "                                       / np.nansum(_d[_s])))\n"
            "        print(f'   {_et:16s} {int(_cual.sum()):8d}'\n"
            "              + ''.join(f'{np.median(_v):11.2%} ' for _v in _vals))\n"
            "    print()\n"
            "    if ES_COMBINADO:\n"
            "        print('Es el cubo que la cadena ajusta, con el modelo TAL CUAL lo entregó')\n"
            "        print('C1: la mediana azimutal de estos paneles ES el mapa de la §7.')\n"
            "    else:\n"
            "        print(f'Es UNA exposición, con dato y modelo igualados en energía dentro de')\n"
            "        print(f'r<={R_NORM:.0f} px: lo que se juzga es la FORMA — cómo reparte esa misma')\n"
            "        print('energía entre el núcleo y el halo—, no la escala de flujo.')\n"
            "elif ES_PSFAO:\n"
            "    print('ningún bin del CSV encontró su bin en el cubo elegido:'\n"
            "          ' no hay paneles que dibujar.')\n"
            "else:\n"
            "    print('sin CSV de psfao: esta sección no aplica a la forma Moffat.')"
        ),
        md(
            "## 8 · ¿Tiene estructura azimutal?\n\n"
            "Todo lo anterior promedia en ángulo, así que **por construcción no puede ver** una "
            "elongación, la refracción diferencial o una vibración: las tres dejan el residuo "
            "repartido de forma distinta según la dirección. Aquí se mira el residuo en sectores "
            "a los radios que importan.\n\n"
            "Si la amplitud azimutal es pequeña frente al residuo medio, el término que falta es "
            "**azimutalmente simétrico**, y eso descarta de golpe a la refracción diferencial "
            "—que tiene una dirección, la del ángulo paraláctico— como explicación principal."
        ),
        code(
            "if ES_PSFAO:\n"
            "    _N_SECT = 12\n"
        ...
        md(
            ...
            "4. **No dice que el híbrido esté mal calculado.** La §6 mide que, tal y como se "
            "guarda, no sirve para el halo —el suavizado que lo hace seguro frente al compañero "
            "es el que lo estropea a 25–32 px—, no que la rama híbrida sea un error. Con otra "
            "escala de suavizado sería otra cosa, y eso **no se prueba aquí**.\n"
            "5. **No propone hacerlo viajar.** Meter un término radial en `psf_model.json` "
            "obliga a que `evaluate_psf_model` lo evalúe, y eso mueve todo lo que consume la "
            "PSF de C1.\n"
            "6. **Los paneles de la §7.d sobre una exposición no juzgan la escala de flujo.** "
            "Dato y modelo se igualan en energía dentro de `R_NORM`, así que lo que ahí se ve es "
            "**forma**: que una exposición sea más o menos brillante no se puede leer en esa "
            "figura, y el dipolo del núcleo es el resto del centrado a píxel entero. Tampoco "
            "dice que el modelo de la cadena esté mal por no describir una exposición: está "
            "ajustado al **combinado**, que no tiene la PSF de ninguna de ellas.\n\n"
            "**Lo que queda por hacer, y ya no es de este notebook:** con el residuo del anillo "
            "cambiando de signo entre el azul y el rojo, y siendo simétrico en ángulo, el "
            "siguiente paso es **pedirle el halo al ajuste** —pesar radialmente el χ², o "
            "ajustar contra `F(r, λ)` en vez de contra la imagen— que es la hipótesis 2 del "
            "traspaso del 2026-08-13, la única de las tres que sigue en pie sin tocar."
        ),
    ]
...
def build_psfhalo_cells(mb, target, run_id):
    ...
    return [
        ...
        md(
            ...
            "> **Cuidado con este párrafo: el pedestal resultó NO ser este error.** Lo que sigue "
            "midiendo esta sección es real —el pedestal existe y se mide aquí— pero atribuirlo al "
            "déficit del halo es lo que la §10 falsa, con la medida del 2026-08-29. Se conserva "
            "porque es el hilo por el que se llegó, no porque la atribución fuera correcta."
        ),
        code(
        ...
        md(
            ...
            "Todo lo anterior mide un déficit **real** del modelo en el halo. Lo que **no** es "
            "cierto es la atribución con la que se cerraba la §9: que ese déficit fuera la causa "
            "del pedestal. `docs/2026-08-29_frente4_pedestal_y_overfit.md`, sonda reproducible en "
            "`scripts/psffit_pedestal_probe.py`.\n\n"
            "**El anillo no predice el pedestal, y va al revés.** ROXs 42B b tiene el anillo del "
            "modelo entregado **9× peor** que ROXs 12 b (61.5 % contra 6.7 %) y **no tiene "
            "pedestal** (+0.40 σ contra +4.86 σ). La celda de abajo comprueba las dos cantidades "
            "para *este* objeto, en vivo.\n\n"
            "**Lo que sí lo causa es la rigidez del fondo del ajuste.** `psf_pair_design` lleva "
            "constante y **dos gradientes lineales**; lo que el modelo deja en el anillo tiene "
            "curvatura a la escala del disco de ajuste, y lo que los lineales no absorben se lo "
            "lleva la amplitud del compañero. Con términos cuadráticos el pedestal de ROXs 12 b "
            "pasa de +91.09 a +10.66 — de **+4.86 σ a +0.29 σ**: el **88 %** del pedestal es el "
            "fondo.\n\n"
            "**Y no se puede quitar así, porque se lleva la señal.** Medido con el exceso del "
            "compañero sobre su propia nula partido por el ruido entre posiciones, los diseños "
            "alternativos **empeoran en los dos objetos**: en ROXs 12 b la SNR del compañero pasa "
            "de **+3.14** a +1.04 (fondo cuadrático) o +1.70 (término de halo de 1 gdl). El "
            "cuadrático se lleva el 88 % del pedestal **y el 68 % de la señal**.\n\n"
            "Es **estructural**: el compañero y los controles están **al mismo radio**, y sobre un "
            "disco de 12 px una fuente puntual y un fondo curvo son casi degenerados. El pedestal "
            "**no es un sesgo del estimador: es el nivel de referencia local**, y restarlo tiene "
            "un sitio correcto que no es el ajuste — la **estandarización de E4 v4**, que resta la "
            "mediana de las nulas medida al mismo radio, **después** de extraer y sin coste en "
            "señal."
        ),
        code(
        ...
        md(
            ...
            "> Lo que sí queda cerrado, con medida y no con argumento: el déficit del halo **no** lo "
            "crea la combinación, **no** es el radio de normalización, **no** es la ponderación y "
            "**no** es el cielo — y **no** es la causa del pedestal (§10).\n\n"
            "> **Lo que sigue abierto es el halo mismo**, no el pedestal: el modelo reproduce sólo "
            "~28 % del cromatismo medido y falla en el azul. Eso no lo arregla nada de lo probado "
            "aquí, y sigue mordiendo a los métodos que usan el modelo como fondo espacial."
        ),
    ]
