...
def build_c2_cells(mb, target, run_id):
    ...
    cells = [
        ...
        code(
            ...
            "          f' recoge el {100 / float(np.nanmedian(apcorr)) / float(np.nanmedian(FACT)):.2f}%'\n"
            ...
            "    ax.set_title(f'{w:.0f} Å — la caja {_s}×{_s} recoge {100 * CAJAS[w][CAJAS_N.index(_s)]:.2f}%'\n"
            ...
            "for w in LAMBDAS:\n"
            "    ax2.plot(CAJAS_N, 100 * CAJAS[w], 'o-', ms=3, lw=1.0, label=f'{w:.0f} Å')\n"
            ...
            "             ha='right', va='center')\n"
            "for w in LAMBDAS:\n"
            "    if np.isfinite(N50[w]):\n"
            "        ax2.plot([N50[w]], [50], 'o', ms=4, color='tab:red')\n"
            "ax2.set_xscale('log'); ax2.set_xlabel('lado de la caja N [px]')\n"
            "ax2.set_ylabel('luz capturada [%]'); ax2.legend(fontsize=7)\n"
