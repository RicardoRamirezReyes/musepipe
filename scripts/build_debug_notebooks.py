...
def build_c1_cells(mb, target, run_id):
    ...
    return [
        ...
        code(
            ...
            "    _clave = _clave_cache()\n"
            "    _guardado = None\n"
            "    if CACHE_MAPA.exists() and not RECALCULAR_MAPA:\n"
            "        try:\n"
            "            _guardado = json.loads(CACHE_MAPA.read_text(encoding='utf-8'))\n"
            "        except ValueError:\n"
            "            _guardado = None\n"
            "        if _guardado is not None and _guardado.get('cache_key') != _clave:\n"
            "            print('la caché está pero su clave no coincide (cubos, bins, radios o')\n"
            "            print('la matemática copiada han cambiado): se recalcula.')\n"
            "            _guardado = None\n"
            "    if _guardado is not None:\n"
            "        FILAS_MAPA = _guardado['filas']\n"
            "        GEOM_MAPA = _guardado.get('geom', {})\n"
            "        print(f'caché leída: {len(FILAS_MAPA)} filas de {CACHE_MAPA}')\n"
            "    else:\n"
            "        _nb = len(range(0, len(BINS_PSFAO), PASO_BINS_MAPA))\n"
            "        print(f'{len(OBS_C1)} cubos × ~{_nb} bins × 2 formas'\n"
            "              f' = ~{_nb * len(OBS_C1)} ajustes Psfao a ~4.5 s, más los de'\n"
            "              ' Moffat y la lectura. Esto se paga UNA vez: va a la caché.')\n"
            "        _t0 = _time.time()\n"
            "        for _k, (_etq, _p) in enumerate(OBS_C1, 1):\n"
            "            _tc = _time.time()\n"
            "            _f, _g = _ajusta_un_cubo(_etq, _p)\n"
            "            FILAS_MAPA.extend(_f); GEOM_MAPA[_etq] = _g\n"
            "            _malos = sum(1 for r in _f if r['status'] != 'ok')\n"
            "            _t = _time.time() - _t0\n"
            "            print(f'  [{_k:2d}/{len(OBS_C1)}] {_etq:10s} {len(_f):3d} filas,'\n"
            "                  f' {_malos:2d} sin converger, {_time.time() - _tc:5.1f} s'\n"
            "                  f'  (total {_t / 60:.1f} min)')\n"
            "            if _k == 1 and len(OBS_C1) > 1:\n"
            "                # El ETA de arriba solo cuenta los ajustes de Psfao. Con un\n"
            "                # cubo medido ya se puede dar el de verdad.\n"
            "                print(f'      -> a este ritmo, {_t * len(OBS_C1) / 60:.0f} min'\n"
            "                      f' para los {len(OBS_C1)}')\n"
            "        if LEGACY:\n"
            "            print('\\nrun marcado `legacy`: NO se escribe la caché.')\n"
            "        else:\n"
            "            CACHE_MAPA.parent.mkdir(parents=True, exist_ok=True)\n"
            "            CACHE_MAPA.write_text(json.dumps(\n"
            "                {'cache_key': _clave, 'geom': GEOM_MAPA, 'filas': FILAS_MAPA},\n"
        ...
        md(
            ...
            "Las dos dispersiones, para cada parámetro:\n\n"
            "* **entre exposiciones, a λ fija** — MAD entre cubos de una misma columna, mediana "
            "sobre las columnas. Es el ruido del ajuste: la misma PSF medida N veces.\n"
            "* **en λ, dentro de un cubo** — MAD de las diferencias entre bins contiguos, "
            "mediana sobre los cubos. Es lo que el modelo cromático interpreta como señal.\n\n"
            "El cociente es el veredicto. **≲ 1** quiere decir que los saltos entre bins "
            "contiguos no superan al ruido de una medida repetida: la estructura en λ es ruido "
            "de ajuste y suavizar en λ no borra nada. **≫ 1** quiere decir que la estructura "
            "sobrevive a las repeticiones y es cromática de verdad.\n\n"
            "Cuidado con una asimetría real: una exposición suelta tiene ~1/N de la profundidad "
            "del combinado, así que su ruido es **mayor** que el del ajuste de la cadena. El "
            "cociente es por eso una cota **conservadora** contra suavizar: si aun así sale "
            "≫ 1, la estructura es sólida.\n\n"
            "Dos correcciones que la tabla aplica y sin las cuales engaña:\n\n"
        ...
        code(
            ...
            "        print(f'\\n{forma}')\n"
            "        print(f'{\"parámetro\":>12s}{\"|mediana|\":>13s}{\"entre exp.\":>13s}'\n"
            "              f'{\"en λ\":>13s}{\"cociente\":>11s}{\"pegado\":>9s}   veredicto')\n"
            "        for campo in campos:\n"
            "            m = _rejilla(forma, campo)\n"
            ...
            "            # entre exposiciones, columna a columna\n"
            "            entre = float(np.ma.median(_mad(m, eje=0, periodo=per)))\n"
            "            # En λ: diferencias entre bins contiguos, cubo a cubo. La resta\n"
            "            # va explícita porque `np.ma.diff` devuelve los huecos como NaN\n"
            "            # sin enmascararlos, y entonces la mediana sale NaN entera.\n"
            "            dif = _envuelve(m[:, 1:] - m[:, :-1], per)\n"
            "            enlam = float(np.ma.median(_mad(dif, eje=1, periodo=per)))\n"
            "            if not np.isfinite(entre) or not np.isfinite(enlam):\n"
            "                continue\n"
            "            peg = _pegados(m)\n"
            "            coc = enlam / entre if entre > 0 else np.inf\n"
            "            # Un parámetro clavado en su límite repite el MISMO número en\n"
            "            # muchas celdas: su dispersión entre exposiciones se va a cero y\n"
            "            # el cociente se dispara sin que haya nada medido. Le pasa a `C`,\n"
            "            # que sale con 45 % de celdas pegadas, 0.00 % entre exposiciones\n"
            "            # y un cociente de seis cifras que no significa nada.\n"
            "            if peg > 0.25 or entre / esc < 1e-3:\n"
            "                v = 'DEGENERADO: sin dispersión que medir, el cociente no vale'\n"
            "            else:\n"
            "                v = ('estructura en λ real' if coc > 2\n"
            "                     else 'indistinguible del ruido' if coc < 1\n"
            "                     else 'ambiguo')\n"
            "            print(f'{campo:>12s}{esc:13.4g}{100 * entre / esc:12.2f}%'\n"
            "                  f'{100 * enlam / esc:12.2f}%{coc:11.2f}{100 * peg:8.0f}%   {v}')\n"
