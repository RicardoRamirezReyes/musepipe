...
def build_c1_cells(mb, target, run_id):
    ...
    return [
        ...
        code(
            "import time\n\n\n"
            "def _fit_fijo(img, var, samp, x0, fijos):\n"
            "    \"\"\"`fit_bin` con parámetros bloqueados. Misma máscara y mismos pesos.\"\"\"\n"
            ...
            "                     weights=np.where(_m, 1 / np.clip(var, 1e-6, None), 0.0),\n"
            "                     flux_bck=(True, True), fixed=fixed, max_nfev=400)\n"
            ...
            "            float(res.cost))\n\n\n"
            "def _pasada(fijos, x0_de, etiqueta):\n"
            ...
            "        par, ring, cost = _fit_fijo(np.nanmedian(CUBE_SUB[_sub], axis=0),\n"
            "                                    np.nanmedian(STAT_SUB[_sub], axis=0),\n"
            "                                    float(SYSTEM.samp(mid * 1e-10)),\n"
            "                                    list(x0_de(mid)), fijos)\n"
        ...
        md(
            ...
            "3. **Si P1 y P2 no mueven el cromatismo**, el déficit del halo **no es un problema "
            "de ajuste**: es de modelo. Y si P2 (la física correcta, Fried) lo empeora, quiere "
            "decir que los ajustes libres estaban **fingiendo** parte del cromatismo con una "
            "`r0` más inclinada de lo que permite la atmósfera.\n"
            "4. El barrido de `p` pone el número: si hace falta un exponente muy por encima de "
            "6/5 para llegar al dato, ninguna re-parametrización honesta de Psfao va a "
            "reproducir ese halo, y la salida es la (a) de la §13 de `apcorr_debug` —imponer la "
            "forma medida— o añadirle al modelo un término que hoy no tiene.\n\n"
